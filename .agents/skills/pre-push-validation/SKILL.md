...
- **Code examples**: `npm run validate:code-examples`, `npm run check:react-outputs`, `npm run test:react-outputs`, `npm run test:swift-runner-contract`, `npm run test:swift-repl`, `npm run test:swift-catalog-contract`, `npm run test:swift-catalog-coverage`, and `npm run check:swift-catalog-coverage`.
...
- **Coding-problem catalog changes**: Run `npm run test:swift-catalog-contract`, `npm run sync:swift-catalog-coverage`, then the catalog coverage tests and sync check. Run `npm run test:swift-catalog-compile` when canonical Swift helpers, fixtures, or source rules change and a Swift compiler is available. Use `npm run validate:swift-catalog-coverage` when complete Swift parity is the acceptance criterion. That validation is intentionally red during the tracked migration.
