...
- **Code examples**: `npm run validate:code-examples`, `npm run validate:twelve-factor-topic`, `npm run check:react-outputs`, `npm run test:react-outputs`, `npm run test:mcp-companion`, `npm run build:mcp-companion`, `npm run test:swift-runner-contract`, `npm run test:swift-runner-service`, `npm run test:swift-repl`, `npm run test:coding-problem-tab-contracts`, `npm run test:coding-problem-vectors`, `npm run check:coding-problem-vectors`, `npm run test:swift-catalog-contract`, `npm run test:swift-catalog-coverage`, and `npm run check:swift-catalog-coverage`.
...
- **Twelve-Factor topic or reference releases**: Run `npm run sync:twelve-factor-examples` after changing a source range or tag, then `npm run validate:twelve-factor-topic`. The validator requires network access to compare 36 excerpts with their immutable GitHub release sources.
- **MCP companion changes**: Install from the repository root with `npm ci`, then run `npm run test:mcp-companion` and `npm run build:mcp-companion`. The site and companion share the root lockfile.
- **Components or page scripts**: Run custom page validation.
- **Swift runner contract, local service, or REPL changes**: Run the deterministic Swift contract, service, and component tests, plus the Swift browser validator when browser behavior, approach execution, accessibility, or layout changes.
...
- **Coding-problem catalog changes**: Run `npm run test:coding-problem-tab-contracts` and `npm run validate:code-examples`. The source contract rejects repeated language tabs, duplicate runner IDs, and generic starter code inside completed approaches. Run the coding-problem vector test and sync check, then `npm run test:swift-catalog-contract`, `npm run sync:swift-catalog-coverage`, the catalog coverage tests, and the manifest sync check. Swift approach tabs stay static and coding-problem pages must not render `SwiftRepl`. Run `npm run test:coding-problem-vector-fixtures` when vector codecs, renderers, or proof fixtures change and all four language toolchains are available. Run `npm run test:swift-catalog-compile` when Swift catalog sources, canonical helpers, fixtures, or source rules change and a Swift compiler is available. The command compiles every migrated Swift source and executes completed approaches under time and output limits. Use `npm run validate:swift-catalog-coverage` when complete Swift parity is the acceptance criterion. That validation is intentionally red during the tracked migration.
