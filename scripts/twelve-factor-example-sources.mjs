...
const sourceRanges = {
  1: {
    TypeScript: ["package.json", 6, 22, "json"],
    Python: ["pyproject.toml", 23, 26, "toml"],
    Go: ["internal/commands/root.go", 11, 20, "go"],
  },
  2: {
    TypeScript: ["README.md", 15, 16, "bash"],
    Python: ["README.md", 16, 17, "bash"],
    Go: ["README.md", 39, 42, "bash"],
  },
  3: {
    TypeScript: ["src/config.ts", 13, 26, "typescript"],
    Python: ["src/order_service/config.py", 12, 22, "python"],
    Go: ["internal/config/config.go", 12, 21, "go"],
  },
  4: {
    TypeScript: ["src/queue.ts", 23, 42, "typescript"],
    Python: ["src/order_service/web.py", 20, 27, "python"],
    Go: ["internal/commands/web.go", 43, 57, "go"],
  },
  5: {
    TypeScript: ["Dockerfile", 1, 17, "dockerfile"],
    Python: ["Dockerfile", 1, 14, "dockerfile"],
    Go: ["Dockerfile", 1, 14, "dockerfile"],
  },
  6: {
    TypeScript: ["src/store.ts", 46, 75, "typescript"],
    Python: ["src/order_service/store.py", 86, 112, "python"],
    Go: ["internal/store/store.go", 36, 70, "go"],
  },
  7: {
    TypeScript: ["src/web.ts", 11, 28, "typescript"],
    Python: ["src/order_service/web.py", 46, 54, "python"],
    Go: ["internal/commands/web.go", 56, 66, "go"],
  },
  8: {
    TypeScript: ["src/queue.ts", 58, 75, "typescript"],
    Python: ["src/order_service/worker.py", 67, 88, "python"],
    Go: ["internal/commands/worker.go", 67, 84, "go"],
  },
  9: {
    TypeScript: ["src/web.ts", 30, 47, "typescript"],
    Python: ["src/order_service/worker.py", 37, 54, "python"],
    Go: ["internal/commands/web.go", 68, 82, "go"],
  },
  10: {
    TypeScript: ["tests/integration/services.test.ts", 20, 31, "typescript"],
    Python: ["tests/integration/test_services.py", 33, 39, "python"],
    Go: ["tests/integration/services_test.go", 17, 38, "go"],
  },
  11: {
    TypeScript: ["src/telemetry.ts", 21, 44, "typescript"],
    Python: ["src/order_service/telemetry.py", 20, 45, "python"],
    Go: ["internal/telemetry/log.go", 47, 54, "go"],
  },
  12: {
    TypeScript: ["src/admin.ts", 7, 20, "typescript"],
    Python: ["src/order_service/admin.py", 13, 39, "python"],
    Go: ["internal/commands/admin.go", 15, 48, "go"],
  },
};

export const examples = factors.flatMap((factor) =>
  Object.entries(sourceRanges[factor.number]).map(
    ([language, [sourcePath, startLine, endLine, codeLanguage]]) => {
