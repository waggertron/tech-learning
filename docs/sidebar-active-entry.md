...
2. The sidebar opens the active link's ancestor groups and centers the link when it falls outside the visible sidebar area.

The script does not move the sidebar when the active link is already visible. This preserves the reader's nearby navigation context.

## Implementation contract

- `src/scripts/sidebar-active-entry.ts` owns ancestor expansion and scroll positioning.
- `src/components/Head.astro` loads the behavior on every page.
- `src/styles/custom.css` owns the visual marker.
- `scripts/validate-custom-pages.mjs` checks the current link, open ancestors, nonzero sidebar scroll, visible position, yellow gradient, text contrast, and absence of an inset border in Chromium.
