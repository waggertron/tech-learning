...
- **2026-07-08: React example output views**: Added generated output panels after every Modern React TSX or TypeScript example. Component examples are transpiled and rendered with React server rendering, local companion modules live under `_react-example-modules/`, and non-component examples use explicit result panels. Evidence: `scripts/sync-react-example-outputs.mjs`, `tests/react-example-output-views.test.mjs`, `docs/react-example-output-views.md`, `src/content/docs/posts/_react-example-modules/`, `src/styles/custom.css`, React post files.
