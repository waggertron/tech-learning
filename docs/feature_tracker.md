...
- **2026-07-08: React example output views**: Added static output panels after every Modern React TSX or TypeScript example, plus generator, targeted test, and maintenance documentation. Evidence: `scripts/sync-react-example-outputs.mjs`, `tests/react-example-output-views.test.mjs`, `docs/react-example-output-views.md`, `src/styles/custom.css`, React post files.
