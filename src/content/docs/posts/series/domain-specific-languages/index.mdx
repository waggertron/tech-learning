...
description: "A planned series that defines DSLs, compares build and adopt choices, maps tooling, and uses business examples to decide when a domain language pays for itself."
sidebar:
  hidden: true
---
...
This page is the working plan and research base for a long-form series on DSLs. It covers the definition, business cases, creation process, concrete examples, gotchas, tool choices, adoption criteria, and operating model.
...
- **Vocabulary**: The nouns and verbs come from the domain, not from generic implementation detail. `discount`, `coverage`, `segment`, `approval_limit`, and `escalation_window` mean something to the business.
- **Structure**: Valid programs have a recognizable shape. A pricing rule might require a name, condition, action, effective date, owner, and test cases.
- **Semantic model**: The language maps text, JSON, or API calls to domain objects and decisions. Syntax is only notation for that model.
- **Validation**: Invalid combinations can be rejected before production. The language can say that a discount cannot be negative, that a condition references an unknown field, or that a rule lacks an owner.
- **Execution**: The language runs through an interpreter, compiler, rule engine, code generator, or host-language API with clear behavior.
...
The research behind this plan converges on a few practical points:

- Martin Fowler's [DSL Guide](https://martinfowler.com/dsl.html) separates internal DSLs, external DSLs, interpretation, and code generation. That distinction matters because each option changes cost, tooling, runtime behavior, and who can author the rules.
- Fowler's [DSL Boundary](https://martinfowler.com/bliki/DslBoundary.html) argues that good DSLs are limited in scope and capability. That is the key test. If the language keeps adding general-purpose features, the design is losing its domain boundary.
- Fowler's [Business Readable DSL](https://martinfowler.com/bliki/BusinessReadableDSL.html) is the most important organizational warning. The sweet spot is often business-readable, not business-writable. Domain experts should understand and review the logic. Engineers still need to harden it, test it, debug it, and operate it.
- Fowler's [Syntactic Noise](https://martinfowler.com/bliki/SyntacticNoise.html) reframes syntax design around domain text versus punctuation. Removing punctuation is not the goal. The goal is notation that leaves the domain decision easy to read.
- Fowler's [Refactoring to an Adaptive Model](https://martinfowler.com/articles/refactoring-adaptive-model.html) shows the bridge between ordinary code and a DSL: move duplicated decision logic into a data model first, then decide whether a DSL is needed to populate that model.
- Strumenta's [external DSL guide](https://tomassetti.me/domain-specific-languages/) emphasizes collaboration with domain experts, faster feedback, and tool support. The insight to keep is that a DSL is not only parser work. It needs editing, validation, examples, and user education.
- [Langium](https://langium.org/) and [JetBrains MPS](https://www.jetbrains.com/mps/) show what serious language tooling includes: typed ASTs, language servers, validation, completion, quick fixes, projectional editing, and code generation.
- [ANTLR](https://www.antlr.org/) and [Chevrotain](https://chevrotain.io/docs/tutorial/step1_lexing.html) show the parser-generator path. The parser gets you structured text. It does not give you semantic validation, governance, or a production runtime by itself.
- The paper [Design Guidelines for Domain Specific Languages](https://arxiv.org/abs/1409.2378) is a useful corrective: language design is error-prone and time-consuming even when tools make the mechanics easier.
...
- **Pricing and discounts**: Promotions, loyalty tiers, eligibility windows, stacking rules, and regional exceptions. The business value comes from changing offers without redeploying every service that calculates prices.
- **Loan and insurance eligibility**: Underwriting rules, risk bands, coverage limits, approval thresholds, and audit requirements. The value comes from versioned decisions and traceable explanations.
- **Authorization and policy-as-code**: Access rules separated from application code. The value comes from central policy ownership and consistent enforcement across services.
- **Workflow routing**: Support-ticket queues, approval chains, escalation rules, SLA routing, and exception handling. The value comes from making operational policy visible.
- **Infrastructure guardrails**: Deployment policies, compliance checks, resource limits, region restrictions, and security rules. The value comes from rejecting unsafe changes before they ship.
- **Data mapping and transformation**: Event normalization, partner integrations, JSON transformations, and field-mapping rules. The value comes from reducing bespoke adapter code.
- **Test scenario descriptions**: Business-readable acceptance cases, state-machine test paths, and fixture generation. The value comes from keeping tests close to the behavior they describe.
- **Code and config generation**: Repetitive domain models that produce API schemas, database mappings, validation rules, or client SDK fragments. The value comes from one source of truth.
...
- Marketing changes thresholds twice a week.
- Legal requires an audit trail for every discount decision.
- Support needs to explain why a customer did or did not qualify.
- Mobile, checkout, billing, and analytics all need the same logic.
- Some rules are active only in certain countries, channels, or date ranges.
...
## Creation example 1: internal TypeScript DSL
...
## Creation example 2: JSON rule model
...
- Unknown fields can fail validation before deploy.
- `effectiveFrom` can be checked against rollout policy.
- `owner` can be required.
- Examples can become golden tests.
- The rule can be rendered in a business UI without asking users to edit raw JSON.
...
## Creation example 3: external text DSL
...
- Actions must obey business limits.
- Rule priority or conflict behavior must be explicit.
- Date windows must not create accidental gaps or overlaps.
- Every rule needs examples that exercise the expected path.

## Creation example 4: policy DSL adoption
...
- **Language design surface**: Every keyword, operator, precedence rule, and edge case becomes a compatibility decision.
- **Tooling burden**: Parsers, validators, syntax highlighting, formatters, docs, fixtures, migrations, and error messages need clear ownership.
- **Debugging cost**: A custom interpreter creates a new runtime model. Logs must explain domain decisions, not parser internals.
- **Versioning pressure**: Stored rules outlive deployments. The language needs compatibility rules, migration paths, and deprecation policy.
- **Security risk**: A DSL that can touch I/O, secrets, network calls, host objects, mutation, or unbounded loops can become arbitrary code execution in disguise.
- **Shadow-platform risk**: Teams may start building application behavior inside the DSL because the normal release process is slower.
- **False readability**: Business-readable syntax is useful. Business-writable logic without tests, previews, ownership, and rollback creates fragile production behavior.
...
- **Use normal code** when engineers own the logic, changes are rare, and one service owns the decision.
- **Use typed configuration** when the domain is declarative, the shape is simple, and validation matters more than custom notation.
- **Use CEL or JsonLogic** when you need bounded expressions over declared data and want to avoid host-language execution.
- **Use Rego or Cedar** when the domain is authorization, compliance, or policy evaluation and an existing policy model fits.
- **Use JSONata or JMESPath** when the domain is JSON selection, mapping, or integration transformation.
- **Use an internal DSL** when engineers author rules but the code needs domain-level readability and structured outputs.
- **Use a parser generator** when custom text syntax is justified and the team can own semantic validation.
- **Use a language workbench** when editor experience, code generation, cross-language integration, and long-lived language evolution justify the platform cost.

## Production checklist

A production DSL is not done when it parses.

- **Schema and type model**: Every readable input field has a declared type, owner, null behavior, and compatibility policy.
- **Semantic validation**: Rules fail before deploy when they reference unknown fields, invalid operators, unsupported actions, bad dates, conflicting priorities, or unsafe behavior.
- **Golden tests**: Each rule has concrete examples with expected decisions and reason traces.
- **Versioning**: Stored rules carry language version, rule version, owner, effective dates, and migration status.
- **Preview and dry run**: Authors can test a rule against sample, historical, or staged data before publish.
- **Review workflow**: Changes carry ticket, approver, rollback plan, and release notes when the domain requires it.
- **Runtime isolation**: Evaluation is deterministic, bounded, and separated from arbitrary host access.
- **Observability**: Decisions emit rule IDs, versions, matched clauses, output actions, latency, and failure reasons.
- **Migration tools**: Language changes can upgrade old rules or keep old versions running safely.
- **Documentation**: The language has examples, counterexamples, glossary, operator reference, and troubleshooting guidance.

## How the series fits together

The series should move from definition to operation. Each post should depend on the previous one and remove one major source of ambiguity.
...
That order matters. If the series starts with parser code, it teaches the wrong reflex. The first reflex should be domain modeling.
