...
| --- | --- | --- | --- | --- |
| TypeScript | [`v1.0.2`](https://github.com/waggertron/twelve-factor-typescript/tree/v1.0.2) | `npm run start:web`, `npm run start:worker`, `npm run admin -- migrate --target 001` | `3101` | `npm run verify` |
| Python | [`v1.0.2`](https://github.com/waggertron/twelve-factor-python/tree/v1.0.2) | `web`, `worker`, `admin migrate --target 001` | `3102` | `uv run python scripts/check_twelve_factor.py` |
| Go | [`v1.0.3`](https://github.com/waggertron/twelve-factor-go/tree/v1.0.3) | `orders web`, `orders worker`, `orders admin migrate --target 001` | `3103` | `./scripts/check-twelve-factor.sh` |
...
**Verify it:** Pick a running web, worker, and admin process. Recover the application name, release identifier, source revision, and build target for each. All process types in one release should point to the same application revision. [See Factor I: Codebase examples in TypeScript, Python, and Go](./examples-factors-01-04/#factor-i-codebase).
...
**Verify it:** Install and test in an empty environment with only the manifest and lockfile available. Confirm the runtime artifact contains every required dependency and starts without compiler or registry access. [See Factor II: Dependencies examples in TypeScript, Python, and Go](./examples-factors-01-04/#factor-ii-dependencies).
...
**Verify it:** Start with missing and malformed values. The process should reject them before readiness without printing secret contents. Change a valid deploy setting without rebuilding the artifact. [See Factor III: Config examples in TypeScript, Python, and Go](./examples-factors-01-04/#factor-iii-config).
...
**Verify it:** Point a release at a fresh compatible resource without editing source. Run the same contract suite against both attachments and confirm that an incompatible provider fails visibly. [See Factor IV: Backing services examples in TypeScript, Python, and Go](./examples-factors-01-04/#factor-iv-backing-services).
...
**Verify it:** Accept an order, terminate that instance, and read the order through another instance. Deliver the same job twice and observe one durable effect. [See Factor VI: Processes examples in TypeScript, Python, and Go](./examples-factors-05-08/#factor-vi-processes).
...
**Verify it:** Increase worker replicas without changing the web fleet. Confirm the queue drains faster, each worker respects its concurrency limit, and total work stays within database and downstream capacity. [See Factor VIII: Concurrency examples in TypeScript, Python, and Go](./examples-factors-05-08/#factor-viii-concurrency).
...
**Verify it:** Measure time to readiness. Send termination during an HTTP request and a queued job, then confirm readiness fails first, accepted work finishes or is retried safely, and the process exits within its deadline. Force-kill a worker to test the path that signal handlers cannot protect. [See Factor IX: Disposability examples in TypeScript, Python, and Go](./examples-factors-09-12/#factor-ix-disposability).
...
**Verify it:** Submit one failing request and reconstruct its path from the external event stream using release, request, and trace identifiers. Repeat with a synthetic sensitive marker and confirm it never reaches collected output. [See Factor XI: Logs examples in TypeScript, Python, and Go](./examples-factors-09-12/#factor-xi-logs).
...
Codex reads repository `AGENTS.md` instructions before work and can load repository skills from `.agents/skills`. The [official AGENTS.md guide](https://learn.chatgpt.com/docs/agent-configuration/agents-md) and [official skills guide](https://learn.chatgpt.com/docs/build-skills) describe those discovery rules. A `SKILL.md` can guide factor analysis, focused checks, and reporting, but executable tests must decide whether a contract passed.

Claude Code reads project instructions from `CLAUDE.md` and project skills from `.claude/skills`. Anthropic's [official project-memory guide](https://code.claude.com/docs/en/memory) recommends importing an existing `AGENTS.md` from `CLAUDE.md` when both tools share a contract. Its [official skills guide](https://code.claude.com/docs/en/skills) documents project skill discovery and description-based activation. These files guide behavior; repository checks enforce the result.

### TypeScript harness at `v1.0.2`

[Root `AGENTS.md`](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/AGENTS.md), [root `CLAUDE.md`](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/CLAUDE.md), [Codex skill](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/.agents/skills/twelve-factor-app/SKILL.md), [Claude skill](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/.claude/skills/twelve-factor-app/SKILL.md), [factor checklist](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/.agents/skills/twelve-factor-app/references/factor-checklist.md), and [`check-twelve-factor.mjs`](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/scripts/check-twelve-factor.mjs).

### Python harness at `v1.0.2`

[Root `AGENTS.md`](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/AGENTS.md), [root `CLAUDE.md`](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/CLAUDE.md), [Codex skill](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/.agents/skills/twelve-factor-app/SKILL.md), [Claude skill](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/.claude/skills/twelve-factor-app/SKILL.md), [factor checklist](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/.agents/skills/twelve-factor-app/references/factor-checklist.md), and [`check_twelve_factor.py`](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/scripts/check_twelve_factor.py).

### Go harness at `v1.0.3`

[Root `AGENTS.md`](https://github.com/waggertron/twelve-factor-go/blob/v1.0.3/AGENTS.md), [root `CLAUDE.md`](https://github.com/waggertron/twelve-factor-go/blob/v1.0.3/CLAUDE.md), [Codex skill](https://github.com/waggertron/twelve-factor-go/blob/v1.0.3/.agents/skills/twelve-factor-app/SKILL.md), [Claude skill](https://github.com/waggertron/twelve-factor-go/blob/v1.0.3/.claude/skills/twelve-factor-app/SKILL.md), [factor checklist](https://github.com/waggertron/twelve-factor-go/blob/v1.0.3/.agents/skills/twelve-factor-app/references/factor-checklist.md), and [`check-twelve-factor.sh`](https://github.com/waggertron/twelve-factor-go/blob/v1.0.3/scripts/check-twelve-factor.sh).
...
- [Docker](../docker/), the artifact and process boundary used by the reference services
- [Kubernetes](../kubernetes/), one platform that can schedule, route, configure, and stop those processes
- [GitOps](../gitops/), reconciliation and release promotion after the application contract is defined
- [Secrets, keys, and tokens](../secrets-keys-tokens/), credential types and secure delivery into deploy configuration
- [Scalability](../../system-design/scalability/), capacity, bottleneck, partitioning, and replication models beyond process concurrency
