...
[Factor IX: Disposability TypeScript source](https://github.com/waggertron/twelve-factor-typescript/blob/v1.0.2/src/web.ts#L30-L47)
  ...
  <TabItem label="Python">
    Dramatiq consumers pause before the worker drains. The configured deadline bounds the pause and stop sequence, then the broker and store close through the shared readiness wrapper.
...
[Factor IX: Disposability Python source](https://github.com/waggertron/twelve-factor-python/blob/v1.0.2/src/order_service/worker.py#L37-L54)
  ...
  <TabItem label="Go">
    The web command waits for cancellation, drops readiness, emits a draining event, and calls `http.Server.Shutdown` with a fixed timeout.
