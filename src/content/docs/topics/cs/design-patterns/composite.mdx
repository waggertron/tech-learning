...
| Natural fit for any domain with inherent tree structure: UI widgets, org charts, file systems, scene graphs | Operations that only make sense on leaves (like `getContent()`) pollute the shared interface or require casting |
