...
The editor below is preloaded with the practice scaffold from `703-kth-largest-element-in-stream.py`. Fill in the body and click **Run tests** to execute. The first run downloads Python in the browser (~10 MB, one time per browser).

<PythonRepl code={practiceCode} id="kth-largest-element-in-a-stream" />
