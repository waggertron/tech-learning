...
The editor below is preloaded with the practice scaffold from `572-subtree-of-another-tree.py`. Fill in the body and click **Run tests** to execute. The first run downloads Python in the browser (~10 MB, one time per browser).

<PythonRepl code={practiceCode} id="subtree-of-another-tree" />
