...
The editor below is preloaded with the practice scaffold from `001-two-sum.py`. Fill in the body of `two_sum` and click **Run tests** to execute. The first run downloads Python in the browser (~10 MB, one time per browser).

<PythonRepl code={practiceCode} id="two-sum" />
