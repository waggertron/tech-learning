...
final class Solution {
    func levelOrder(_ root: TreeNode?) -> [[Int]] {
        guard let root else { return [] }
        var result: [[Int]] = [], queue = [root], read = 0
        while read < queue.count {
            let end = queue.count
            var level: [Int] = []
            while read < end { let node = queue[read]; read += 1; level.append(node.val); if let left = node.left { queue.append(left) }; if let right = node.right { queue.append(right) } }
            result.append(level)
        }
        return result
    }
