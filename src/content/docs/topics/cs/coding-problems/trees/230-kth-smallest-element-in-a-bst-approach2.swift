...
final class Solution { func kthSmallest(_ root: TreeNode?, _ k: Int) -> Int { var remaining = k, answer = 0; func visit(_ node: TreeNode?) { guard let node, remaining > 0 else { return }; visit(node.left); remaining -= 1; if remaining == 0 { answer = node.val; return }; visit(node.right) }; visit(root); return answer } }
