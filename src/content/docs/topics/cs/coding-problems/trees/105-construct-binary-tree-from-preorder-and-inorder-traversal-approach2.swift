...
final class Solution {
    func buildTree(_ preorder: [Int], _ inorder: [Int]) -> TreeNode? { let positions = Dictionary(uniqueKeysWithValues: inorder.enumerated().map { ($0.element, $0.offset) }); var preorderIndex = 0; func build(_ left: Int, _ right: Int) -> TreeNode? { guard left <= right else { return nil }; let value = preorder[preorderIndex]; preorderIndex += 1; guard let middle = positions[value] else { return nil }; let node = TreeNode(value); node.left = build(left, middle - 1); node.right = build(middle + 1, right); return node }; return build(0, inorder.count - 1) }
}
