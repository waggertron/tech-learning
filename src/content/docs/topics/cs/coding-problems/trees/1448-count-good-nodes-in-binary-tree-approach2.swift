...
final class Solution { func goodNodes(_ root: TreeNode?) -> Int { guard let root else { return 0 }; func count(_ node: TreeNode?, _ maximum: Int) -> Int { guard let node else { return 0 }; return (node.val >= maximum ? 1 : 0) + count(node.left, max(maximum, node.val)) + count(node.right, max(maximum, node.val)) }; return count(root, root.val) } }
