...
final class Solution { func rob(_ root: TreeNode?) -> Int { var memo: [ObjectIdentifier: Int] = [:]; func solve(_ node: TreeNode?) -> Int { guard let node else { return 0 }; let key = ObjectIdentifier(node); if let cached = memo[key] { return cached }; let take = node.val + solve(node.left?.left) + solve(node.left?.right) + solve(node.right?.left) + solve(node.right?.right); let skip = solve(node.left) + solve(node.right); let best = max(take, skip); memo[key] = best; return best }; return solve(root) } }
