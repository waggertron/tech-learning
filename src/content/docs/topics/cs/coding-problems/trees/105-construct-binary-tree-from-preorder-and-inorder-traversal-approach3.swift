...
final class Solution {
    func buildTree(_ preorder: [Int], _ inorder: [Int]) -> TreeNode? { guard let first = preorder.first else { return nil }; let root = TreeNode(first); var stack = [root], inorderIndex = 0; for value in preorder.dropFirst() { var node = stack[stack.count - 1]; if node.val != inorder[inorderIndex] { node.left = TreeNode(value); if let left = node.left { stack.append(left) } } else { while let last = stack.last, last.val == inorder[inorderIndex] { node = stack.removeLast(); inorderIndex += 1 }; node.right = TreeNode(value); if let right = node.right { stack.append(right) } } }; return root }
}
