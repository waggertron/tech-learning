...
final class Solution {
    func levelOrder(_ root: TreeNode?) -> [[Int]] {
        guard let root else { return [] }
        var result: [[Int]] = [], queue: [(TreeNode, Int)] = [(root, 0)], read = 0
        while read < queue.count { let entry = queue[read]; read += 1; if entry.1 == result.count { result.append([]) }; result[entry.1].append(entry.0.val); if let left = entry.0.left { queue.append((left, entry.1 + 1)) }; if let right = entry.0.right { queue.append((right, entry.1 + 1)) } }
        return result
    }
}

func runTests() {
    // TEST_VECTORS_BEGIN sha256:8a73bf46e321600dbaff1afbf35bf0b458c78789fa77a80e932cef54f73215c1
    let treeArgument1Case1 = makeTree([3, 9, 20, nil, nil, 15, 7])
    expectEqual(Solution().levelOrder(treeArgument1Case1), [[3], [9, 20], [15, 7]], "three-levels")
    let treeArgument1Case2 = makeTree([1, 2, nil, 3])
    expectEqual(Solution().levelOrder(treeArgument1Case2), [[1], [2], [3]], "left-skewed")
    let treeArgument1Case3 = makeTree([1])
    expectEqual(Solution().levelOrder(treeArgument1Case3), [[1]], "single-node")
    let treeArgument1Case4 = makeTree([])
    expectEqual(Solution().levelOrder(treeArgument1Case4), [], "empty-tree")
