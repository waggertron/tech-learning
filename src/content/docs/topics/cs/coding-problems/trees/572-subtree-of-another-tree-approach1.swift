...
final class Solution { func isSubtree(_ root: TreeNode?, _ subRoot: TreeNode?) -> Bool { guard let root else { return subRoot == nil }; return same(root, subRoot) || isSubtree(root.left, subRoot) || isSubtree(root.right, subRoot) }; private func same(_ left: TreeNode?, _ right: TreeNode?) -> Bool { guard let left, let right else { return left == nil && right == nil }; return left.val == right.val && same(left.left, right.left) && same(left.right, right.right) } }
