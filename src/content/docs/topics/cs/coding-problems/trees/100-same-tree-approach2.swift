...
final class Solution {
    func isSameTree(_ p: TreeNode?, _ q: TreeNode?) -> Bool {
        ...
        while index < queue.count {
            let pair = queue[index]
            index += 1
            if pair.0 == nil || pair.1 == nil { if pair.0 != nil || pair.1 != nil { return false }; continue }
            guard let left = pair.0, let right = pair.1, left.val == right.val else { return false }
