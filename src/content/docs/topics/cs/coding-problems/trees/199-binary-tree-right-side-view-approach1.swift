...
final class Solution { func rightSideView(_ root: TreeNode?) -> [Int] { guard let root else { return [] }; var result: [Int] = [], queue = [root], read = 0; while read < queue.count { let end = queue.count; while read < end { let node = queue[read]; read += 1; if read == end { result.append(node.val) }; if let left = node.left { queue.append(left) }; if let right = node.right { queue.append(right) } } }; return result } }
