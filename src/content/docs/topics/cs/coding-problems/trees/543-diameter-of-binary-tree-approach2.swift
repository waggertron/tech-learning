...
final class Solution { func diameterOfBinaryTree(_ root: TreeNode?) -> Int { var diameter = 0; func height(_ node: TreeNode?) -> Int { guard let node else { return 0 }; let left = height(node.left), right = height(node.right); diameter = max(diameter, left + right); return 1 + max(left, right) }; _ = height(root); return diameter } }
