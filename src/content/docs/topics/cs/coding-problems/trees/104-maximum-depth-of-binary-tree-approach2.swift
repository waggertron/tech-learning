...
final class Solution {
    func maxDepth(_ root: TreeNode?) -> Int { guard let root else { return 0 }; var queue = [root], read = 0, depth = 0; while read < queue.count { let end = queue.count; while read < end { let node = queue[read]; read += 1; if let left = node.left { queue.append(left) }; if let right = node.right { queue.append(right) } }; depth += 1 }; return depth }
}

func runTests() {
    // TEST_VECTORS_BEGIN sha256:37b53c3c590d988622bc0863fbda113af4a69cfd61a89fd88bd3b75e3f4cb2ec
    let treeArgument1Case1 = makeTree([3, 9, 20, nil, nil, 15, 7])
    expectEqual(Solution().maxDepth(treeArgument1Case1), 3, "three-levels")
    let treeArgument1Case2 = makeTree([1, nil, 2])
    expectEqual(Solution().maxDepth(treeArgument1Case2), 2, "right-child")
    let treeArgument1Case3 = makeTree([1])
    expectEqual(Solution().maxDepth(treeArgument1Case3), 1, "single-node")
    let treeArgument1Case4 = makeTree([])
    expectEqual(Solution().maxDepth(treeArgument1Case4), 0, "empty-tree")
