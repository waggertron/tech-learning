...
final class Solution { func invertTree(_ root: TreeNode?) -> TreeNode? { guard let root else { return nil }; var stack = [root]; while let node = stack.popLast() { let left = node.left; node.left = node.right; node.right = left; if let left = node.left { stack.append(left) }; if let right = node.right { stack.append(right) } }; return root } }
