...
final class Solution {
    func levelOrder(_ root: TreeNode?) -> [[Int]] {
        var result: [[Int]] = []
        func visit(_ node: TreeNode?, _ depth: Int) { guard let node else { return }; if depth == result.count { result.append([]) }; result[depth].append(node.val); visit(node.left, depth + 1); visit(node.right, depth + 1) }
        visit(root, 0)
        return result
    }
}
