...
final class Solution {
    ...
    func combinationSum(_ candidates: [Int], _ target: Int) -> [[Int]] {
        ...
        func search(_ start: Int, _ remaining: Int, _ current: [Int]) { if remaining == 0 { result.append(current); return }; for index in start..<values.count { let value = values[index]; if value > remaining { break }; search(index, remaining - value, current + [value]) } }
