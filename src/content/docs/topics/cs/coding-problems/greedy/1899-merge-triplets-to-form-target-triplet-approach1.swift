...
final class Solution {
    func mergeTriplets(_ triplets: [[Int]], _ target: [Int]) -> Bool {
        for mask in 1..<(1 << triplets.count) {
            var merged = [0, 0, 0]
            for index in triplets.indices where mask & (1 << index) != 0 {
                for channel in 0..<3 { merged[channel] = max(merged[channel], triplets[index][channel]) }
            }
