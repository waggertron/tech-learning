...
func canJump(nums []int) bool {
    ...
    f = func(i int) bool {                              // L1: called exponential times naively
        if i >= len(nums)-1 { return true }             // L2: base case O(1)
        for step := 1; step <= nums[i]; step++ {        // L3: try each step
            if f(i + step) { return true }              // L4: recursive call
        }
        return false
    }
...
func canJump(nums []int) bool {
    n := len(nums)                                         // L1: O(1)
    dp := make([]bool, n)                                  // L2: O(n)
    dp[n-1] = true                                         // L3: O(1)
    for i := n - 2; i >= 0; i-- {                         // L4: outer loop, n-1 iterations
        furthest := i + nums[i]
        if furthest > n-1 { furthest = n - 1 }
        for j := i + 1; j <= furthest; j++ {              // L5: inner loop, up to n per i
...
func canJump(nums []int) bool {
    ...
    for i, x := range nums {                              // L2: single pass, n iterations
        if i > maxReach { return false }                   // L3: O(1)
        if i+x > maxReach { maxReach = i + x }            // L4: O(1), update reach
        if maxReach >= len(nums)-1 { return true }         // L5: O(1), early exit
    }
    return true
}
```
