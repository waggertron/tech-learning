...
final class Solution {
    func checkValidString(_ s: String) -> Bool {
        ...
        func search(_ index: Int, _ open: Int) -> Bool {
            ...
            let key = index * (characters.count + 1) + open
            if let cached = memo[key] { return cached }
            let result: Bool
            if characters[index] == "(" { result = search(index + 1, open + 1) }
            else if characters[index] == ")" { result = search(index + 1, open - 1) }
            else { result = search(index + 1, open) || search(index + 1, open + 1) || search(index + 1, open - 1) }
            memo[key] = result
            return result
        }
        return search(0, 0)
