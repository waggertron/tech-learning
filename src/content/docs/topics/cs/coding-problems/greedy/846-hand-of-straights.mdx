...
import practiceCodeGo from './846-hand-of-straights.go?raw';
import practiceCodeSwift from './846-hand-of-straights.swift?raw';
import approach1SwiftCode from './846-hand-of-straights-approach1.swift?raw';
import approach2SwiftCode from './846-hand-of-straights-approach2.swift?raw';
import approach3SwiftCode from './846-hand-of-straights-approach3.swift?raw';
...
import approach2GoCode from './846-hand-of-straights-approach2.go?raw';
...
import approach3GoCode from './846-hand-of-straights-approach3.go?raw';
...
<Tabs>
  ...
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="hand-of-straights-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="846-hand-of-straights-swift" />
  </TabItem>
</Tabs>
...
func isNStraightHand(hand []int, groupSize int) bool {
    ...
    n := len(hand)
    var comb func(start, k int, chosen []int) bool
    comb = func(start, k int, chosen []int) bool {
        if k == 0 {
            group := make([]int, len(chosen))
            copy(group, chosen)
            sort.Ints(group)                                        // L1: C(n, k) groups
            for i := 1; i < len(group); i++ {
                if group[i]-group[i-1] != 1 { return false }
            }
            var rem []int
            used := map[int]bool{}
            for _, idx := range chosen { used[idx] = true }
            for i := 0; i < n; i++ {
                if !used[i] { rem = append(rem, hand[i]) }
            }
            return isNStraightHand(rem, groupSize)                  // L2: recurse on rest
        }
        for i := start; i <= n-k; i++ {
            if comb(i+1, k-1, append(chosen, i)) { return true }
        }
        return false
    }
    return comb(0, groupSize, nil)
}
```
</TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="846-hand-of-straights-approach1-swift" />
  </TabItem>
</Tabs>
...
```go
func isNStraightHand(hand []int, groupSize int) bool {
    if len(hand)%groupSize != 0 { return false }                    // L1: O(1) quick reject
    counts := make(map[int]int)
    for _, v := range hand { counts[v]++ }                          // L2: O(n)
    keys := make([]int, 0, len(counts))
    for k := range counts { keys = append(keys, k) }
    sort.Ints(keys)                                                 // L3: O(u log u)
    for _, x := range keys {
        c := counts[x]                                              // L4: O(1)
        if c == 0 { continue }
        for k := 0; k < groupSize; k++ {                           // L5: O(group_size) per value
            if counts[x+k] < c { return false }                    // L6: O(1)
            counts[x+k] -= c                                       // L7: O(1)
        }
    }
    return true
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="846-hand-of-straights-approach2-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="hand-of-straights-approach2-go" />
  </TabItem>
</Tabs>
...
func isNStraightHandHeap(hand []int, groupSize int) bool {
    if len(hand)%groupSize != 0 { return false }                    // L1: O(1)
    counts := make(map[int]int)
    for _, v := range hand { counts[v]++ }                          // L2: O(n)
    keys := make([]int, 0, len(counts))
    for k := range counts { keys = append(keys, k) }
    sort.Ints(keys)                                                 // L3: O(u)
    ki := 0
    for ki < len(keys) {                                            // L5: outer loop
        x := keys[ki]
        if counts[x] == 0 { ki++; continue }                       // L7: skip exhausted
        for k := 0; k < groupSize; k++ {                           // L8: O(group_size) per start
            if counts[x+k] == 0 { return false }
            counts[x+k]--                                          // L9: O(1)
        }
    }
    return true
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="846-hand-of-straights-approach3-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="hand-of-straights-approach3-go" />
  </TabItem>
</Tabs>
