...
final class Solution {
    func mergeTriplets(_ triplets: [[Int]], _ target: [Int]) -> Bool {
        ...
        for triplet in triplets where zip(triplet, target).allSatisfy({ $0 <= $1 }) {
            for channel in 0..<3 where triplet[channel] == target[channel] { matched[channel] = true }
        }
        return matched.allSatisfy { $0 }
    }
}
