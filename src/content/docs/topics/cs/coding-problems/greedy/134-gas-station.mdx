...
console.assert(canCompleteCircuit([1, 2, 3, 4, 5], [3, 4, 5, 1, 2]) === 3);
console.assert(canCompleteCircuit([2, 3, 4], [3, 4, 3]) === -1);
console.assert(canCompleteCircuit([1], [1]) === 0);     // single station, exact balance
console.assert(canCompleteCircuit([5], [4]) === 0);     // single station with surplus
console.assert(canCompleteCircuit([2, 0, 1], [0, 1, 2]) === 0);  // start at 0
...
- [Greedy Algorithms](../../../coding-concepts/greedy-algorithms/), local-choice tactics for solving optimization and reachability problems when a provable invariant protects the future.
