...
final class Solution {
    func maxProfit(_ prices: [Int]) -> Int {
        ...
        var minimums = Array(repeating: first, count: prices.count)
        for index in prices.indices.dropFirst() { minimums[index] = min(minimums[index - 1], prices[index]) }
        return prices.indices.reduce(0) { max($0, prices[$1] - minimums[$1]) }
    }
}
