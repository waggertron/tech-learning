...
import practiceCode from './424-longest-repeating-character-replacement.py?raw';
import practiceCodeTs from './424-longest-repeating-character-replacement.ts?raw';
import practiceCodeGo from './424-longest-repeating-character-replacement.go?raw';
import practiceCodeSwift from './424-longest-repeating-character-replacement.swift?raw';
import approach1SwiftCode from './424-longest-repeating-character-replacement-approach1.swift?raw';
import approach2SwiftCode from './424-longest-repeating-character-replacement-approach2.swift?raw';
import approach3SwiftCode from './424-longest-repeating-character-replacement-approach3.swift?raw';
import approach3PyCode from './424-longest-repeating-character-replacement-approach3.py?raw';
import approach3TsCode from './424-longest-repeating-character-replacement-approach3.ts?raw';
import approach3GoCode from './424-longest-repeating-character-replacement-approach3.go?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="longest-repeating-character-replacement-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="longest-repeating-character-replacement-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="longest-repeating-character-replacement-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="424-longest-repeating-character-replacement-swift" />
  </TabItem>
</Tabs>
...
function characterReplacement(s: string, k: number): number {
    ...
    for (let i = 0; i < n; i++) {                            // L1: outer loop, n iterations
        ...
        for (let j = i; j < n; j++) {                        // L2: inner loop, O(n) per outer
            counts.set(s[j], (counts.get(s[j]) ?? 0) + 1);  // L3: O(1) incremental update
            const length = j - i + 1;
            const maxCount = Math.max(...counts.values());    // L4: O(k) max over counts
            if (length - maxCount <= k)
                best = Math.max(best, length);                // L5: O(1)
        }
    }
    return best;
}
...
func characterReplacement(s string, k int) int {
	...
	for i := 0; i < n; i++ {                              // L1: outer loop, n iterations
		...
		for j := i; j < n; j++ {                          // L2: inner loop, O(n) per outer
			counts[s[j]]++                                // L3: O(1) incremental update
			length := j - i + 1
			maxCount := 0
			for _, v := range counts {                    // L4: O(k) max over counts
				if v > maxCount {
					maxCount = v
				}
			}
			if length-maxCount <= k && length > best {   // L5: O(1)
				best = length
			}
		}
	}
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="424-longest-repeating-character-replacement-approach1-swift" />
  </TabItem>
</Tabs>
...
function characterReplacement(s: string, k: number): number {
    ...
    for (const target of chars) {                   // L1: at most 26 targets
        let left = 0, nonTarget = 0;
        for (let right = 0; right < s.length; right++) {  // L2: n iterations per target
            if (s[right] !== target) nonTarget++;   // L3/L4: O(1)
            while (nonTarget > k) {                 // L5: shrink window
                if (s[left] !== target) nonTarget--;// L6: O(1)
                left++;                             // L7: O(1)
            }
...
func characterReplacement(s string, k int) int {
	...
	for target := range seen {                         // L1: at most 26 targets
		left, nonTarget := 0, 0
		for right := 0; right < len(s); right++ {     // L2: n iterations per target
			if s[right] != target {                   // L3/L4: O(1)
				nonTarget++
			}
			for nonTarget > k {                       // L5: shrink window
				if s[left] != target {               // L6: O(1)
					nonTarget--
				}
				left++                               // L7: O(1)
			}
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="424-longest-repeating-character-replacement-approach2-swift" />
  </TabItem>
</Tabs>
...
func characterReplacement(s string, k int) int {
	...
	maxFreq := 0
	best := 0
	for right := 0; right < len(s); right++ {           // L1: outer loop, n iterations
		ch := s[right]
		counts[ch]++                                     // L2: O(1)
		if counts[ch] > maxFreq {                        // L3: O(1) running max
			maxFreq = counts[ch]
		}
		for (right-left+1)-maxFreq > k {                // L4: shrink if too many replacements
			counts[s[left]]--                            // L5: O(1)
			left++                                       // L6: O(1)
		}
		if right-left+1 > best {                         // L7: O(1)
			best = right - left + 1
		}
	}
	return best
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="424-longest-repeating-character-replacement-approach3-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach3PyCode} id="longest-repeating-approach3-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach3TsCode} id="longest-repeating-approach3-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="longest-repeating-approach3-go" />
  </TabItem>
</Tabs>
...
console.assert(characterReplacement("ABAB", 2) === 4);
console.assert(characterReplacement("AABABBA", 1) === 4);
console.assert(characterReplacement("A", 0) === 1);
console.assert(characterReplacement("AAAA", 2) === 4);
console.assert(characterReplacement("ABCDE", 1) === 2);
console.assert(characterReplacement("AABBA", 2) === 5);
...
- [Monotonic Queue](../../../coding-concepts/monotonic-queue/), deque tactics for maintaining a window minimum or maximum as the window slides.
