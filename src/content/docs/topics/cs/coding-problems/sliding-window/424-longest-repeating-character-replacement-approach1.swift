...
final class Solution {
    func characterReplacement(_ s: String, _ k: Int) -> Int {
        ...
        for start in characters.indices {
            var counts: [Character: Int] = [:], maximum = 0
            for end in start..<characters.count {
                counts[characters[end], default: 0] += 1
                maximum = max(maximum, counts[characters[end], default: 0])
                let length = end - start + 1
                if length - maximum <= k { best = max(best, length) }
            }
        }
        return best
    }
