...
final class Solution {
    func findSubstring(_ s: String, _ words: [String]) -> [Int] {
        ...
        for offset in 0..<min(wordLength, text.count) {
            var left = offset
            var used = 0
            ...
            while right + wordLength <= text.count {
                let word = String(text[right..<(right + wordLength)])
                right += wordLength

                guard let limit = required[word] else {
                ...
                used += 1
                while window[word, default: 0] > limit {
                    let outgoing = String(text[left..<(left + wordLength)])
                    window[outgoing, default: 0] -= 1
                    used -= 1
                    left += wordLength
                }
