...
final class Solution {
    func maxSlidingWindow(_ nums: [Int], _ k: Int) -> [Int] {
        var heap = BinaryHeap<(value: Int, index: Int)> { left, right in
            left.value == right.value ? left.index > right.index : left.value > right.value
        }
        var result: [Int] = []
        for index in nums.indices {
            heap.insert((nums[index], index))
            while let top = heap.peek, top.index <= index - k { _ = heap.removeRoot() }
            if index >= k - 1, let top = heap.peek { result.append(top.value) }
        }
        return result
    }
}
