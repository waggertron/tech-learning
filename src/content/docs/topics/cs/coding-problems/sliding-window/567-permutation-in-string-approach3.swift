...
final class Solution {
    func checkInclusion(_ s1: String, _ s2: String) -> Bool {
        ...
        let required = pattern.reduce(into: [Character: Int]()) { $0[$1, default: 0] += 1 }
        var window: [Character: Int] = [:], satisfied = 0
        for right in text.indices {
            let added = text[right]
            window[added, default: 0] += 1
            if window[added] == required[added] { satisfied += 1 }
            if right >= pattern.count {
                let removed = text[right - pattern.count]
                if window[removed] == required[removed] { satisfied -= 1 }
                window[removed, default: 0] -= 1
            }
            if right >= pattern.count - 1 && satisfied == required.count { return true }
        }
        return false
    }
}
