...
final class Solution {
    func getSum(_ a: Int, _ b: Int) -> Int {
        let left = UInt32(bitPattern: Int32(a))
        let right = UInt32(bitPattern: Int32(b))
        var result: UInt32 = 0
        var carry: UInt32 = 0

        for bit in 0..<32 {
            let leftBit = (left >> bit) & 1
            let rightBit = (right >> bit) & 1
            let sumBit = leftBit ^ rightBit ^ carry
            result |= sumBit << bit
            carry = (leftBit & rightBit) | (carry & (leftBit ^ rightBit))
        }
