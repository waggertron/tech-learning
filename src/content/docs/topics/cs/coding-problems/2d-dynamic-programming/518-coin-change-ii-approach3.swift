...
final class Solution {
    func change(_ amount: Int, _ coins: [Int]) -> Int {
        var dp = Array(repeating: 0, count: amount + 1); dp[0] = 1
        for coin in coins where coin <= amount { if coin <= amount { for value in coin...amount { dp[value] += dp[value - coin] } } }; return dp[amount]
    }
}
