...
final class Solution {
    func findTargetSumWays(_ nums: [Int], _ target: Int) -> Int {
        var memo: [String: Int] = [:]
        func solve(_ index: Int, _ sum: Int) -> Int { if index == nums.count { return sum == target ? 1 : 0 }; let key = "\(index):\(sum)"; if let value = memo[key] { return value }; let value = solve(index + 1, sum + nums[index]) + solve(index + 1, sum - nums[index]); memo[key] = value; return value }
        return solve(0, 0)
    }
}
