...
final class Solution {
    func maxCoins(_ nums: [Int]) -> Int {
        ...
        func solve(_ values: [Int]) -> Int { if values.isEmpty { return 0 }; var best = 0; for i in values.indices { let left = i == 0 ? 1 : values[i - 1]; let right = i == values.count - 1 ? 1 : values[i + 1]; var next = values; next.remove(at: i); best = max(best, left * values[i] * right + solve(next)) }; return best }
        return solve(nums)
    }
}
