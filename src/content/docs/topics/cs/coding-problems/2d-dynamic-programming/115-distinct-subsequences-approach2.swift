...
final class Solution {
    func numDistinct(_ s: String, _ t: String) -> Int {
        let source = Array(s), target = Array(t); var memo: [String: Int] = [:]
        func solve(_ i: Int, _ j: Int) -> Int { if j == target.count { return 1 }; if i == source.count { return 0 }; let key = "\(i):\(j)"; if let value = memo[key] { return value }; let value = solve(i + 1, j) + (source[i] == target[j] ? solve(i + 1, j + 1) : 0); memo[key] = value; return value }
        return solve(0, 0)
    }
}
