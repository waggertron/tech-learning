...
final class Solution {
    func change(_ amount: Int, _ coins: [Int]) -> Int {
        var memo: [String: Int] = [:]
        func solve(_ index: Int, _ remaining: Int) -> Int { if remaining == 0 { return 1 }; if index == coins.count || remaining < 0 { return 0 }; let key = "\(index):\(remaining)"; if let value = memo[key] { return value }; let value = solve(index, remaining - coins[index]) + solve(index + 1, remaining); memo[key] = value; return value }
