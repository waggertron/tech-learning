...
final class Solution {
    func longestIncreasingPath(_ matrix: [[Int]]) -> Int {
        let rows = matrix.count, cols = matrix[0].count, directions = [(1,0),(-1,0),(0,1),(0,-1)]
        func dfs(_ row: Int, _ col: Int) -> Int { var best = 1; for (dr, dc) in directions { let r = row + dr, c = col + dc; if r >= 0 && r < rows && c >= 0 && c < cols && matrix[r][c] > matrix[row][col] { best = max(best, 1 + dfs(r, c)) } }; return best }
