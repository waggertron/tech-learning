...
final class Solution {
    func maxProfit(_ prices: [Int]) -> Int {
        var memo: [String: Int] = [:]
        func solve(_ day: Int, _ holding: Bool) -> Int { if day >= prices.count { return 0 }; let key = "\(day):\(holding)"; if let value = memo[key] { return value }; let value = holding ? max(solve(day + 1, true), prices[day] + solve(day + 2, false)) : max(solve(day + 1, false), -prices[day] + solve(day + 1, true)); memo[key] = value; return value }
