...
final class Solution {
    func findTargetSumWays(_ nums: [Int], _ target: Int) -> Int {
        let total = nums.reduce(0,+); guard abs(target) <= total && (total + target) % 2 == 0 else { return 0 }; let desired = (total + target) / 2; var dp = Array(repeating: 0, count: desired + 1); dp[0] = 1
        for number in nums { if number <= desired { for sum in stride(from: desired, through: number, by: -1) { dp[sum] += dp[sum - number] } } }; return dp[desired]
    }
}
