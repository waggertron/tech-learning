...
import practiceCode from './329-longest-increasing-path-in-a-matrix.py?raw';
import practiceCodeTs from './329-longest-increasing-path-in-a-matrix.ts?raw';
import practiceCodeGo from './329-longest-increasing-path-in-a-matrix.go?raw';
import approach2PyCode from './329-longest-increasing-path-in-a-matrix-approach2.py?raw';
import approach2TsCode from './329-longest-increasing-path-in-a-matrix-approach2.ts?raw';
import approach2GoCode from './329-longest-increasing-path-in-a-matrix-approach2.go?raw';
import approach3PyCode from './329-longest-increasing-path-in-a-matrix-approach3.py?raw';
import approach3TsCode from './329-longest-increasing-path-in-a-matrix-approach3.ts?raw';
import approach3GoCode from './329-longest-increasing-path-in-a-matrix-approach3.go?raw';
import SwiftRepl from '../../../../../../components/SwiftRepl.astro';
import practiceCodeSwift from './329-longest-increasing-path-in-a-matrix.swift?raw';
import approach1SwiftCode from './329-longest-increasing-path-in-a-matrix-approach1.swift?raw';
import approach2SwiftCode from './329-longest-increasing-path-in-a-matrix-approach2.swift?raw';
import approach3SwiftCode from './329-longest-increasing-path-in-a-matrix-approach3.swift?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="longest-increasing-path-in-a-matrix-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="longest-increasing-path-in-a-matrix-ts" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="329-longest-increasing-path-in-a-matrix-swift" />
  </TabItem>
</Tabs>
...
function longestIncreasingPath(matrix: number[][]): number {
    if (!matrix.length) return 0;                                      // L1: O(1) guard
    const rows = matrix.length, cols = matrix[0].length;               // L2: O(1)
    const dirs = [[1, 0], [-1, 0], [0, 1], [0, -1]];
    function dfs(r: number, c: number): number {
        let best = 1;                                                   // L3: O(1) minimum path
        for (const [dr, dc] of dirs) {
            const nr = r + dr, nc = c + dc;
            if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && matrix[nr][nc] > matrix[r][c])
                best = Math.max(best, 1 + dfs(nr, nc));                // L5: recursive call
        }
        return best;
    }
    let ans = 0;
    for (let r = 0; r < rows; r++)
        for (let c = 0; c < cols; c++)
            ans = Math.max(ans, dfs(r, c));                            // L6: run from every cell
    return ans;
}
```
</TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="329-longest-increasing-path-in-a-matrix-approach1-swift" />
  </TabItem>
</Tabs>
...
function longestIncreasingPath(matrix: number[][]): number {
    ...
    function dfs(r: number, c: number): number {
        const key = r * cols + c;
        if (memo.has(key)) return memo.get(key)!;                      // L3: cache lookup
        let best = 1;                                                   // L4: O(1) minimum path
        for (const [dr, dc] of dirs) {
            const nr = r + dr, nc = c + dc;
            if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && matrix[nr][nc] > matrix[r][c])
                best = Math.max(best, 1 + dfs(nr, nc));                // L6: O(1) with cache
        }
        memo.set(key, best);
        return best;
    }
    ...
    for (let r = 0; r < rows; r++)
        for (let c = 0; c < cols; c++)
            ans = Math.max(ans, dfs(r, c));                            // L7: O(m*n) launches
    return ans;
}
```
</TabItem>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach2PyCode} id="longest-increasing-path-in-a-matrix-approach2-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach2TsCode} id="longest-increasing-path-in-a-matrix-approach2-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="longest-increasing-path-in-a-matrix-approach2-go" />
  </TabItem>

  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="329-longest-increasing-path-in-a-matrix-approach2-swift" />
  </TabItem>
</Tabs>
...
function longestIncreasingPath(matrix: number[][]): number {
    if (!matrix.length) return 0;                                          // L1: O(1) guard
    const rows = matrix.length, cols = matrix[0].length;                   // L2: O(1)
    const inDeg: number[][] = Array.from({ length: rows }, () => new Array(cols).fill(0));  // L3
    const dirs = [[1, 0], [-1, 0], [0, 1], [0, -1]];
    for (let r = 0; r < rows; r++) {                                       // L4: build in-degree
        for (let c = 0; c < cols; c++) {
            for (const [dr, dc] of dirs) {
                const nr = r + dr, nc = c + dc;
                if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && matrix[nr][nc] < matrix[r][c])
    ...
    for (let r = 0; r < rows; r++)
        for (let c = 0; c < cols; c++)
            if (inDeg[r][c] === 0) queue.push([r, c]);                    // L6: seed minima
    let levels = 0, qi = 0;
    while (qi < queue.length) {                                            // L8: BFS levels
        levels++;
        ...
        for (let s = 0; s < size; s++) {
            ...
            for (const [dr, dc] of dirs) {
                const nr = r + dr, nc = c + dc;
                if (nr >= 0 && nr < rows && nc >= 0 && nc < cols && matrix[nr][nc] > matrix[r][c])
                    if (--inDeg[nr][nc] === 0) queue.push([nr, nc]);      // L10+L11
            }
        }
    }
    return levels;
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach3PyCode} id="longest-increasing-path-in-a-matrix-approach3-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach3TsCode} id="longest-increasing-path-in-a-matrix-approach3-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="longest-increasing-path-in-a-matrix-approach3-go" />
  </TabItem>

  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="329-longest-increasing-path-in-a-matrix-approach3-swift" />
  </TabItem>
</Tabs>
...
console.assert(longestIncreasingPath([[1]]) === 1);
console.assert(longestIncreasingPath([[1,1],[1,1]]) === 1);
console.assert(longestIncreasingPath([[1,2,3,4]]) === 4);
