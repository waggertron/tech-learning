...
final class WordDictionary {
    ...
    init() {}
    func addWord(_ word: String) { words.append(word) }
    func search(_ word: String) -> Bool {
        let pattern = Array(word)
        return words.contains { candidate in
            let characters = Array(candidate)
            return characters.count == pattern.count && characters.indices.allSatisfy {
                pattern[$0] == "." || pattern[$0] == characters[$0]
            }
        }
    }
}
