...
final class WordDictionary {
    private var wordsByLength: [Int: [[Character]]] = [:]
    init() {}
    func addWord(_ word: String) { wordsByLength[word.count, default: []].append(Array(word)) }
    func search(_ word: String) -> Bool {
        let pattern = Array(word)
        return wordsByLength[pattern.count, default: []].contains { candidate in
            candidate.indices.allSatisfy { pattern[$0] == "." || pattern[$0] == candidate[$0] }
        }
    }
}
