...
final class Solution {
    func findWords(_ board: [[Character]], _ words: [String]) -> [String] {
        ...
        func search(_ row: Int, _ column: Int, _ node: TrieNode, _ path: String) {
            guard row >= 0, row < board.count, column >= 0, column < board[0].count, !visited[row][column],
                  let child = node.children[board[row][column]] else { return }
            let nextPath = path + String(board[row][column])
            if child.isWord { found.insert(nextPath) }
