...
final class Solution {
    func productExceptSelf(_ nums: [Int]) -> [Int] {
        var result = Array(repeating: 1, count: nums.count), prefix = 1
        for index in nums.indices { result[index] = prefix; prefix *= nums[index] }
        var suffix = 1
        for index in nums.indices.reversed() { result[index] *= suffix; suffix *= nums[index] }
        return result
    }
}
