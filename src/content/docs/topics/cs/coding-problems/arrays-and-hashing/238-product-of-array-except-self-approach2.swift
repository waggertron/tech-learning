...
final class Solution {
    func productExceptSelf(_ nums: [Int]) -> [Int] {
        let count = nums.count; var prefix = Array(repeating: 1, count: count), suffix = prefix
        for index in 1..<count { prefix[index] = prefix[index - 1] * nums[index - 1] }
        for index in stride(from: count - 2, through: 0, by: -1) { suffix[index] = suffix[index + 1] * nums[index + 1] }
