...
final class Solution {
    func topKFrequent(_ nums: [Int], _ k: Int) -> [Int] {
        var counts: [Int: Int] = [:]; for value in nums { counts[value, default: 0] += 1 }; var buckets = Array(repeating: [Int](), count: nums.count + 1)
        for (value, count) in counts { buckets[count].append(value) }
        var result: [Int] = []; for count in stride(from: buckets.count - 1, through: 1, by: -1) { for value in buckets[count].sorted() { result.append(value); if result.count == k { return result } } }
        return result
    }
}
