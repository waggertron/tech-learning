    ...
    func threeSum(_ nums: [Int]) -> [[Int]] {
        ...
        for first in 0..<(values.count - 2) {
            ...
            for second in (first + 1)..<values.count {
                let complement = -values[first] - values[second]
                if seen.contains(complement) {
                    let triplet = [values[first], complement, values[second]]
                    if result.last != triplet { result.append(triplet) }
                }
                seen.insert(values[second])
            }
        }
        result.sort {
