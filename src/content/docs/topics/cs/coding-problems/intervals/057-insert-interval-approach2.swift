...
final class Solution {
    func insert(_ intervals: [[Int]], _ newInterval: [Int]) -> [[Int]] {
        ...
        while low < high { let middle = (low + high) / 2; if intervals[middle][0] < newInterval[0] { low = middle + 1 } else { high = middle } }
        var values = intervals
        values.insert(newInterval, at: low)
        var result: [[Int]] = []
        for interval in values {
            if let last = result.last, interval[0] <= last[1] { result[result.count - 1][1] = max(last[1], interval[1]) }
