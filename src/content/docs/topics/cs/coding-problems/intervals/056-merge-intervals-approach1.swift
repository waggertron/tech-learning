...
final class Solution {
    func merge(_ intervals: [[Int]]) -> [[Int]] {
        ...
        while changed {
            changed = false
            outer: for left in 0..<values.count {
                for right in (left + 1)..<values.count {
                    if max(values[left][0], values[right][0]) <= min(values[left][1], values[right][1]) {
                        values[left] = [min(values[left][0], values[right][0]), max(values[left][1], values[right][1])]
                        values.remove(at: right)
                        changed = true
                        break outer
                    }
                }
