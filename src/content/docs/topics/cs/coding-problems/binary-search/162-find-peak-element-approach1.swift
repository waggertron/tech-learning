...
final class Solution {
    func findPeakElement(_ nums: [Int]) -> Int {
        for index in nums.indices {
            let greaterThanLeft = index == 0 || nums[index] > nums[index - 1]
            let greaterThanRight = index == nums.count - 1 || nums[index] > nums[index + 1]
            if greaterThanLeft && greaterThanRight { return index }
        }
        preconditionFailure("A valid input always contains a peak")
    }
}
