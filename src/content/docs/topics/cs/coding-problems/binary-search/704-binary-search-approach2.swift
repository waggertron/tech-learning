...
final class Solution {
    func search(_ nums: [Int], _ target: Int) -> Int {
        search(nums, target, low: 0, high: nums.count - 1)
    }

    private func search(_ nums: [Int], _ target: Int, low: Int, high: Int) -> Int {
        guard low <= high else { return -1 }
        let middle = low + (high - low) / 2
        if nums[middle] == target { return middle }
        if nums[middle] < target {
            return search(nums, target, low: middle + 1, high: high)
        }
        return search(nums, target, low: low, high: middle - 1)
    }
}
