...
import practiceCode from './033-search-in-rotated-sorted-array.py?raw';
import practiceCodeTs from './033-search-in-rotated-sorted-array.ts?raw';
import practiceCodeGo from './033-search-in-rotated-sorted-array.go?raw';
import practiceCodeSwift from './033-search-in-rotated-sorted-array.swift?raw';
import approach1SwiftCode from './033-search-in-rotated-sorted-array-approach1.swift?raw';
import approach2SwiftCode from './033-search-in-rotated-sorted-array-approach2.swift?raw';
import approach3SwiftCode from './033-search-in-rotated-sorted-array-approach3.swift?raw';
import approach2PyCode from './033-search-in-rotated-sorted-array-approach2.py?raw';
import approach2TsCode from './033-search-in-rotated-sorted-array-approach2.ts?raw';
import approach2GoCode from './033-search-in-rotated-sorted-array-approach2.go?raw';
import approach3PyCode from './033-search-in-rotated-sorted-array-approach3.py?raw';
import approach3TsCode from './033-search-in-rotated-sorted-array-approach3.ts?raw';
import approach3GoCode from './033-search-in-rotated-sorted-array-approach3.go?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="search-in-rotated-sorted-array-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="search-in-rotated-sorted-array-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="search-in-rotated-sorted-array-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="033-search-in-rotated-sorted-array-swift" />
  </TabItem>
</Tabs>
...
function search(nums: number[], target: number): number {
    for (let i = 0; i < nums.length; i++) {  // L1: scan all elements, up to n
...
func search(nums []int, target int) int {
    for i, x := range nums {  // L1: scan all elements, up to n
        if x == target {      // L2: O(1) compare
            return i
        }
    }
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="033-search-in-rotated-sorted-array-approach1-swift" />
  </TabItem>
</Tabs>
...
function search(nums: number[], target: number): number {
    ...
    const pivot = lo;                              // L3: O(1) pivot found
    // Step 2: pick which half to binary-search
    if (pivot === 0 || target < nums[0]) {
        lo = pivot; hi = nums.length - 1;          // L4: O(1) right segment
    } else {
        lo = 0; hi = pivot - 1;                   // L5: O(1) left segment
    }
    while (lo <= hi) {                             // L6: second binary search, O(log n)
        const mid = (lo + hi) >> 1;
...
func search(nums []int, target int) int {
    // Step 1: find rotation pivot (index of min)
    lo, hi := 0, len(nums)-1
    for lo < hi {                              // L2: first binary search, O(log n)
        mid := (lo + hi) / 2
        if nums[mid] > nums[hi] {
            lo = mid + 1
        } else {
    ...
    for lo <= hi {                             // L6: second binary search, O(log n)
        mid := (lo + hi) / 2
        if nums[mid] == target {
            return mid
        }
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="033-search-in-rotated-sorted-array-approach2-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach2PyCode} id="search-in-rotated-sorted-array-approach2-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach2TsCode} id="search-in-rotated-sorted-array-approach2-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="search-in-rotated-sorted-array-approach2-go" />
  </TabItem>
</Tabs>
...
function search(nums: number[], target: number): number {
    ...
    while (lo <= hi) {                               // L2: loop, O(log n) iterations
        ...
        if (nums[lo] <= nums[mid]) {                 // L5: O(1) left-half sorted check
            if (nums[lo] <= target && target < nums[mid]) hi = mid - 1;  // L6
            else lo = mid + 1;                       // L7
        } else {                                     // right half is sorted
            if (nums[mid] < target && target <= nums[hi]) lo = mid + 1;  // L8
            else hi = mid - 1;                       // L9
        }
    }
    return -1;
...
func search(nums []int, target int) int {
    lo, hi := 0, len(nums)-1
    for lo <= hi {                               // L2: loop, O(log n) iterations
        mid := (lo + hi) / 2                   // L3: O(1) midpoint
        if nums[mid] == target {                // L4: O(1) compare
            return mid
        }
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="033-search-in-rotated-sorted-array-approach3-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach3PyCode} id="search-in-rotated-sorted-array-approach3-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach3TsCode} id="search-in-rotated-sorted-array-approach3-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="search-in-rotated-sorted-array-approach3-go" />
  </TabItem>
</Tabs>
...
console.assert(search([4, 5, 6, 7, 0, 1, 2], 3) === -1);   // not present
console.assert(search([1], 0) === -1);                        // single element miss
console.assert(search([1], 1) === 0);                         // single element hit
console.assert(search([3, 1], 1) === 1);                      // small rotated, target on right
console.assert(search([3, 1], 3) === 0);                      // small rotated, target on left
...
func main() {
    if search([]int{4, 5, 6, 7, 0, 1, 2}, 0) != 4  { panic("test 1") }
    if search([]int{4, 5, 6, 7, 0, 1, 2}, 3) != -1 { panic("test 2") } // not present
    if search([]int{1}, 0) != -1                     { panic("test 3") } // single element miss
    if search([]int{1}, 1) != 0                      { panic("test 4") } // single element hit
    if search([]int{3, 1}, 1) != 1                   { panic("test 5") } // small rotated, target on right
    if search([]int{3, 1}, 3) != 0                   { panic("test 6") } // small rotated, target on left
