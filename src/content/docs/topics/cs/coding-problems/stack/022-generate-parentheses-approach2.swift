...
final class Solution {
    func generateParenthesis(_ n: Int) -> [String] {
        var result: [String] = []
        func generate(_ current: String, _ opens: Int, _ closes: Int) {
            if current.count == 2 * n { result.append(current); return }
            if opens < n { generate(current + "(", opens + 1, closes) }
            if closes < opens { generate(current + ")", opens, closes + 1) }
        }
        generate("", 0, 0)
        return result
    }
