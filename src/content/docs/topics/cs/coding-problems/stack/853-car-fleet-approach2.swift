...
final class Solution {
    func carFleet(_ target: Int, _ position: [Int], _ speed: [Int]) -> Int {
        let cars = zip(position, speed).map { (position: $0.0, time: Double(target - $0.0) / Double($0.1)) }.sorted { $0.position > $1.position }
        var fleets = 0
        var slowestAhead = 0.0
        for car in cars where car.time > slowestAhead { fleets += 1; slowestAhead = car.time }
        return fleets
    }
}
