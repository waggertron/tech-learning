...
final class Solution {
    func sumSubarrayMins(_ arr: [Int]) -> Int {
        ...
        for right in 0...arr.count {
            let current = right == arr.count ? Int.min : arr[right]
            while let middle = stack.last, arr[middle] >= current {
                stack.removeLast()
                let left = stack.last ?? -1
                let contribution = ((arr[middle] * (middle - left)) % modulus * (right - middle)) % modulus
                total = (total + contribution) % modulus
            }
            if right < arr.count { stack.append(right) }
        }
        return total
    }
}
