...
final class MinStack {
    ...
    func push(_ value: Int) {
        values.append(value)
        if value <= (minimums.last ?? value) { minimums.append(value) }
    }
    func pop() {
        let removed = values.removeLast()
        if removed == minimums.last { minimums.removeLast() }
    }
    func top() -> Int { values.last ?? 0 }
    func getMin() -> Int { minimums.last ?? 0 }
}
