...
final class MinStack {
    ...
    func push(_ value: Int) { entries.append((value, min(value, entries.last?.minimum ?? value))) }
    func pop() { entries.removeLast() }
    func top() -> Int { entries.last?.value ?? 0 }
    func getMin() -> Int { entries.last?.minimum ?? 0 }
}
