...
final class Solution {
    func canPartition(_ nums: [Int]) -> Bool { let sum = nums.reduce(0, +); if sum % 2 == 1 { return false }; let target = sum / 2; var possible: Set<Int> = [0]; for value in nums { for total in possible.sorted(by: >) where total + value <= target { possible.insert(total + value) } }; return possible.contains(target) }
}
