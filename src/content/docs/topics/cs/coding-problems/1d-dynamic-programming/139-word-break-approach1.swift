...
final class Solution {
    func wordBreak(_ s: String, _ wordDict: [String]) -> Bool { let c = Array(s), words = wordDict.map(Array.init); func solve(_ i: Int) -> Bool { if i == c.count { return true }; for word in words where i + word.count <= c.count { if Array(c[i..<(i + word.count)]) == word && solve(i + word.count) { return true } }; return false }; return solve(0) }
}
