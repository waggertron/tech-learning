...
final class Solution {
    func canPartition(_ nums: [Int]) -> Bool { let sum = nums.reduce(0, +); if sum % 2 == 1 { return false }; let target = sum / 2; func solve(_ i: Int, _ remaining: Int) -> Bool { if remaining == 0 { return true }; if i == nums.count || remaining < 0 { return false }; return solve(i + 1, remaining) || solve(i + 1, remaining - nums[i]) }; return solve(0, target) }
}
