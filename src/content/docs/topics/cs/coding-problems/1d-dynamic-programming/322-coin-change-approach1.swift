...
final class Solution {
    func coinChange(_ coins: [Int], _ amount: Int) -> Int { func solve(_ remaining: Int) -> Int { if remaining == 0 { return 0 }; if remaining < 0 { return Int.max / 4 }; let best = coins.map { solve(remaining - $0) }.min() ?? Int.max / 4; let result = best >= Int.max / 4 ? best : best + 1; return result }; let answer = solve(amount); return answer >= Int.max / 4 ? -1 : answer }
}
