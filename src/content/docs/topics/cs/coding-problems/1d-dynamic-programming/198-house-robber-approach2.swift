...
final class Solution {
    func rob(_ nums: [Int]) -> Int { var memo: [Int: Int] = [:]; func solve(_ i: Int) -> Int { if i >= nums.count { return 0 }; if let v = memo[i] { return v }; let v = max(solve(i + 1), nums[i] + solve(i + 2)); memo[i] = v; return v }; return solve(0) }
}
