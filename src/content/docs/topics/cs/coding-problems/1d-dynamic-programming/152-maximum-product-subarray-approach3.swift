...
final class Solution {
    func maxProduct(_ nums: [Int]) -> Int { var maximum = nums[0], minimum = nums[0], best = nums[0]; for value in nums.dropFirst() { let a = maximum * value, b = minimum * value; maximum = max(value, max(a, b)); minimum = min(value, min(a, b)); best = max(best, maximum) }; return best }
}
