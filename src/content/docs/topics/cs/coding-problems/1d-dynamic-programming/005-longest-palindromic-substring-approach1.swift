...
final class Solution {
    func longestPalindrome(_ s: String) -> String { let c = Array(s); var best: [Character] = []; for i in c.indices { for j in i..<c.count { let part = Array(c[i...j]); if part.count > best.count && part == Array(part.reversed()) { best = part } } }; return String(best) }
}
