...
import practiceCodeTs from './322-coin-change.ts?raw';
import practiceCodeGo from './322-coin-change.go?raw';
import practiceCodeSwift from './322-coin-change.swift?raw';
import approach1SwiftCode from './322-coin-change-approach1.swift?raw';
import approach2SwiftCode from './322-coin-change-approach2.swift?raw';
import approach3SwiftCode from './322-coin-change-approach3.swift?raw';
import approach2PyCode from './322-coin-change-approach2.py?raw';
import approach2TsCode from './322-coin-change-approach2.ts?raw';
import approach2GoCode from './322-coin-change-approach2.go?raw';
import approach3PyCode from './322-coin-change-approach3.py?raw';
import approach3TsCode from './322-coin-change-approach3.ts?raw';
import approach3GoCode from './322-coin-change-approach3.go?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="coin-change-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="coin-change-ts" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="322-coin-change-swift" />
  </TabItem>
</Tabs>
...
function coinChange(coins: number[], amount: number): number {
    function f(n: number): number {                            // L1: define recursive helper
        if (n === 0) return 0;                                // L2: O(1) base case
        if (n < 0) return Infinity;                           // L3: O(1) base case
        let best = Infinity;                                  // L4: O(1)
        for (const c of coins) {                             // L5: loop over denominations
            best = Math.min(best, 1 + f(n - c));             // L6: O(1) + recurse
        }
        return best;
    }
    const result = f(amount);                                // L7: kick off recursion
    return result === Infinity ? -1 : result;                // L8: O(1)
}
```
</TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="322-coin-change-approach1-swift" />
  </TabItem>
</Tabs>
...
function coinChange(coins: number[], amount: number): number {
    ...
    function f(n: number): number {                            // L1: define memoized helper
        if (n === 0) return 0;                                // L2: O(1) base case
        if (n < 0) return Infinity;                           // L3: O(1) base case
        if (memo.has(n)) return memo.get(n)!;
        let best = Infinity;                                  // L4: O(1)
        for (const c of coins) {                             // L5: loop over n denominations
            best = Math.min(best, 1 + f(n - c));             // L6: O(1) + cached sub-call
        }
        memo.set(n, best);
        return best;
    }
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="322-coin-change-approach2-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach2PyCode} id="coin-change-approach2-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach2TsCode} id="coin-change-approach2-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="coin-change-approach2-go" />
  </TabItem>
...
```typescript
function coinChange(coins: number[], amount: number): number {
    const INF = amount + 1;                                    // L1: sentinel
    const dp = new Array(amount + 1).fill(INF);               // L2: O(A) init
    dp[0] = 0;                                                // L3: base case
    for (let i = 1; i <= amount; i++) {                      // L4: outer loop, A iterations
        for (const c of coins) {                             // L5: inner loop, n iterations
            if (c <= i) dp[i] = Math.min(dp[i], dp[i - c] + 1);  // L6/L7: O(1) recurrence
        }
    }
    return dp[amount] !== INF ? dp[amount] : -1;             // L8: O(1)
}
```
</TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="322-coin-change-approach3-swift" />
  </TabItem>
</Tabs>
...
`float('inf')` works too, but `amount + 1` stays an integer and gives a tighter bound. See [Sentinel Values](../../../sentinel-values/) for the general pattern across DP, shortest-path, and search problems.

**Try this approach:**

<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach3PyCode} id="coin-change-approach3-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach3TsCode} id="coin-change-approach3-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="coin-change-approach3-go" />
  </TabItem>
...
console.assert(coinChange([1], 1) === 1);
console.assert(coinChange([2, 5, 10, 1], 27) === 4);
console.assert(coinChange([186, 419, 83, 408], 6249) === 20);
...
- [Sentinel Values](../../../sentinel-values/), the general pattern for "impossible" placeholders in algorithms
