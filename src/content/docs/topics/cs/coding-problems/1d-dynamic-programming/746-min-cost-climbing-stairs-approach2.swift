...
final class Solution {
    func minCostClimbingStairs(_ cost: [Int]) -> Int { var memo: [Int: Int] = [:]; func solve(_ i: Int) -> Int { if i >= cost.count { return 0 }; if let v = memo[i] { return v }; let v = cost[i] + min(solve(i + 1), solve(i + 2)); memo[i] = v; return v }; return min(solve(0), solve(1)) }
}
