...
import practiceCode from './050-pow-x-n.py?raw';
import practiceCodeTs from './050-pow-x-n.ts?raw';
import practiceCodeGo from './050-pow-x-n.go?raw';
import practiceCodeSwift from './050-pow-x-n.swift?raw';
import approach1SwiftCode from './050-pow-x-n-approach1.swift?raw';
import approach2SwiftCode from './050-pow-x-n-approach2.swift?raw';
import approach3SwiftCode from './050-pow-x-n-approach3.swift?raw';
import approach2PyCode from './050-pow-x-n-approach2.py?raw';
import approach2TsCode from './050-pow-x-n-approach2.ts?raw';
import approach2GoCode from './050-pow-x-n-approach2.go?raw';
import approach3PyCode from './050-pow-x-n-approach3.py?raw';
import approach3TsCode from './050-pow-x-n-approach3.ts?raw';
import approach3GoCode from './050-pow-x-n-approach3.go?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="pow-x-n-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="pow-x-n-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="pow-x-n-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="050-pow-x-n-swift" />
  </TabItem>
</Tabs>
...
function myPow(x: number, n: number): number {
    if (n < 0) { x = 1 / x; n = -n; }   // L1-L3: O(1)
    let result = 1.0;
    for (let i = 0; i < n; i++) result *= x;  // L4, L5: loop |n| times
    return result;
}
```
</TabItem>
...
func myPow(x float64, n int) float64 {
    if n < 0 { x = 1 / x; n = -n }   // L1-L3: O(1)
    result := 1.0
    for i := 0; i < n; i++ { result *= x }  // L4, L5: loop |n| times
    return result
}
```
</TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="050-pow-x-n-approach1-swift" />
  </TabItem>
</Tabs>
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="050-pow-x-n-approach2-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach2PyCode} id="pow-x-n-approach2-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach2TsCode} id="pow-x-n-approach2-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="pow-x-n-approach2-go" />
  </TabItem>
</Tabs>
...
func myPow(x float64, n int) float64 {
    ...
    for n != 0 {             // L5: loop log n times
        if n&1 == 1 { result *= x }  // L6: O(1), multiply in if odd bit
        x *= x               // L7: O(1), square x each iteration
        n >>= 1              // L8: O(1), shift to next bit
    }
    return result
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="050-pow-x-n-approach3-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach3PyCode} id="pow-x-n-approach3-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach3TsCode} id="pow-x-n-approach3-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="pow-x-n-approach3-go" />
  </TabItem>
</Tabs>
...
console.assert(Math.abs(myPow(2.0, 10) - 1024.0) < 1e-9);
console.assert(Math.abs(myPow(2.0, -2) - 0.25) < 1e-9);
console.assert(Math.abs(myPow(2.0, 0) - 1.0) < 1e-9);
console.assert(Math.abs(myPow(1.0, 1000000) - 1.0) < 1e-9);
console.assert(Math.abs(myPow(0.0, 5) - 0.0) < 1e-9);
console.assert(Math.abs(myPow(2.0, 1) - 2.0) < 1e-9);
