...
final class DetectSquares {
    ...
    func count(_ point: [Int]) -> Int {
        ...
        var total = 0
        for horizontal in points where horizontal.y == qy && horizontal.x != qx {
            for vertical in points where vertical.x == qx && vertical.y != qy {
                guard abs(horizontal.x - qx) == abs(vertical.y - qy) else { continue }
                total += points.reduce(0) { count, candidate in
                    count + (candidate.x == horizontal.x && candidate.y == vertical.y ? 1 : 0)
                }
            }
        }
        return total
