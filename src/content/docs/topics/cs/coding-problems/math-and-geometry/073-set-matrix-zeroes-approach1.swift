...
final class Solution {
    func setZeroes(_ matrix: inout [[Int]]) {
        ...
        let snapshot = matrix
        for row in snapshot.indices {
            for column in snapshot[row].indices where snapshot[row][column] == 0 {
                for targetRow in matrix.indices { matrix[targetRow][column] = 0 }
                for targetColumn in matrix[row].indices { matrix[row][targetColumn] = 0 }
            }
        }
    }
}
