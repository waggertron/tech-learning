...
function _runTests(): void {
    ...
    // perf
    const bigTimes = Array.from({ length: 199 }, (_, i) => [i + 1, i + 2, 1]);
    const t0 = performance.now();
    networkDelayTime(bigTimes, 200, 1);
    console.log(`perf network-delay-time 200 nodes chain: ${(performance.now() - t0).toFixed(1)}ms`);
