...
function _runTests(): void {
    ...
    // perf
    const airports = Array.from({ length: 50 }, (_, i) => 'A' + String(i).padStart(3, '0'));
    const bigTickets: string[][] = airports.slice(0, 49).map((a, i) => [a, airports[i + 1]]);
    bigTickets.push(['JFK', airports[0]]);
    const t0 = performance.now();
    findItinerary(bigTickets);
    console.log(`perf reconstruct-itinerary 51 tickets linear chain: ${(performance.now() - t0).toFixed(1)}ms`);
