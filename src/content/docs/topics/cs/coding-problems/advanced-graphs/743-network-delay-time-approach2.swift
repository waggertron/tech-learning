...
final class Solution {
    func networkDelayTime(_ times: [[Int]], _ n: Int, _ k: Int) -> Int {
        let infinity = Int.max / 4; var distance = Array(repeating: Array(repeating: infinity, count: n), count: n)
        for i in 0..<n { distance[i][i] = 0 }; for edge in times { distance[edge[0] - 1][edge[1] - 1] = min(distance[edge[0] - 1][edge[1] - 1], edge[2]) }
        for middle in 0..<n { for from in 0..<n where distance[from][middle] < infinity { for to in 0..<n where distance[middle][to] < infinity { distance[from][to] = min(distance[from][to], distance[from][middle] + distance[middle][to]) } } }
        let answer = distance[k - 1].max()!; return answer == infinity ? -1 : answer
    }
}
