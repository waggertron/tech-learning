...
final class Solution {
    func swimInWater(_ grid: [[Int]]) -> Int {
        let n = grid.count, directions = [(1,0),(-1,0),(0,1),(0,-1)]; var best = Array(repeating: Array(repeating: Int.max, count: n), count: n); best[0][0] = grid[0][0]
        var queue = BinaryHeap<(Int,Int,Int)>(hasHigherPriority: { $0.0 < $1.0 }); queue.insert((grid[0][0],0,0))
        while let (time,row,col) = queue.removeRoot() { if row == n-1 && col == n-1 { return time }; if time != best[row][col] { continue }; for (dr,dc) in directions { let r=row+dr,c=col+dc; if r>=0 && r<n && c>=0 && c<n { let candidate=max(time,grid[r][c]); if candidate < best[r][c] { best[r][c]=candidate; queue.insert((candidate,r,c)) } } } }; return -1
    }
}
