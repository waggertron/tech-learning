...
final class Solution {
    func findCheapestPrice(_ n: Int, _ flights: [[Int]], _ src: Int, _ dst: Int, _ k: Int) -> Int {
        let infinity=Int.max/4; var cost=Array(repeating:infinity,count:n); cost[src]=0
        for _ in 0...k { var next=cost; for flight in flights where cost[flight[0]]<infinity { next[flight[1]]=min(next[flight[1]],cost[flight[0]]+flight[2]) }; cost=next }
