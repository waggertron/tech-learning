...
final class Solution {
    func findItinerary(_ tickets: [[String]]) -> [String] {
        ...
        for ticket in tickets { graph[ticket[0], default: []].append(ticket[1]) }
        for airport in graph.keys { graph[airport]!.sort() }
        var nextIndex: [String: Int] = [:]
        var route: [String] = []
        func visit(_ airport: String) {
            let neighbors = graph[airport] ?? []
            while nextIndex[airport, default: 0] < neighbors.count {
                let index = nextIndex[airport, default: 0]
                nextIndex[airport] = index + 1
                visit(neighbors[index])
            }
            route.append(airport)
        }
        visit("JFK"); return route.reversed()
    }
}
