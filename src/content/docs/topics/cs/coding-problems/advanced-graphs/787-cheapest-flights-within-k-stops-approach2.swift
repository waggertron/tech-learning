...
final class Solution {
    func findCheapestPrice(_ n: Int, _ flights: [[Int]], _ src: Int, _ dst: Int, _ k: Int) -> Int {
        var graph=Array(repeating:[(Int,Int)](),count:n); for flight in flights { graph[flight[0]].append((flight[1],flight[2])) }; let infinity=Int.max/4; var distance=Array(repeating:Array(repeating:infinity,count:n),count:k+2); distance[0][src]=0
        var queue=BinaryHeap<(Int,Int,Int)>(hasHigherPriority:{$0.0<$1.0}); queue.insert((0,src,0))
        while let (cost,node,edges)=queue.removeRoot() { if node==dst { return cost }; if edges==k+1 || cost != distance[edges][node] { continue }; for (next,price) in graph[node] { let candidate=cost+price; if candidate<distance[edges+1][next] { distance[edges+1][next]=candidate; queue.insert((candidate,next,edges+1)) } } }; return -1
    }
}
