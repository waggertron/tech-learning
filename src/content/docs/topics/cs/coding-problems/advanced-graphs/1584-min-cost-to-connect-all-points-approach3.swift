...
final class Solution {
    func minCostConnectPoints(_ points: [[Int]]) -> Int {
        let n = points.count; if n <= 1 { return 0 }; var visited = Array(repeating: false, count: n), total = 0, used = 0
        var queue = BinaryHeap<(Int, Int)>(hasHigherPriority: { $0.0 < $1.0 }); queue.insert((0, 0))
        while used < n, let (cost, node) = queue.removeRoot() { if visited[node] { continue }; visited[node] = true; total += cost; used += 1; for next in 0..<n where !visited[next] { let distance = abs(points[node][0] - points[next][0]) + abs(points[node][1] - points[next][1]); queue.insert((distance, next)) } }
        return total
    }
}
