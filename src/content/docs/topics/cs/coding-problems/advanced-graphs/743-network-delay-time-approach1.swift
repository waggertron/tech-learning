...
final class Solution {
    func networkDelayTime(_ times: [[Int]], _ n: Int, _ k: Int) -> Int {
        ...
        if n > 1 { for _ in 0..<(n - 1) { var changed = false; for edge in times where distance[edge[0]] < infinity { let candidate = distance[edge[0]] + edge[2]; if candidate < distance[edge[1]] { distance[edge[1]] = candidate; changed = true } }; if !changed { break } } }
