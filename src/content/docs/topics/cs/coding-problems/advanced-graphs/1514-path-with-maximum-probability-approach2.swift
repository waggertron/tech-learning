...
final class Solution {
    func maxProbability(_ n: Int, _ edges: [[Int]], _ succProb: [Double], _ start: Int, _ end: Int) -> Double {
        ...
        for index in edges.indices { let u = edges[index][0], v = edges[index][1], chance = succProb[index]; graph[u].append((v, chance)); graph[v].append((u, chance)) }
        var best = Array(repeating: 0.0, count: n); best[start] = 1.0
        var queue = BinaryHeap<(Double, Int)>(hasHigherPriority: { $0.0 > $1.0 }); queue.insert((1.0, start))
        while let (chance, node) = queue.removeRoot() { if node == end { return chance }; if chance < best[node] { continue }; for (neighbor, edgeChance) in graph[node] { let candidate = chance * edgeChance; if candidate > best[neighbor] { best[neighbor] = candidate; queue.insert((candidate, neighbor)) } } }
        return 0.0
    }
}
