...
import practiceCodeGo from './025-reverse-nodes-in-k-group.go?raw';
import practiceCodeSwift from './025-reverse-nodes-in-k-group.swift?raw';
import approach1SwiftCode from './025-reverse-nodes-in-k-group-approach1.swift?raw';
import approach2SwiftCode from './025-reverse-nodes-in-k-group-approach2.swift?raw';
import approach3SwiftCode from './025-reverse-nodes-in-k-group-approach3.swift?raw';
import approach2PyCode from './025-reverse-nodes-in-k-group-approach2.py?raw';
import approach2TsCode from './025-reverse-nodes-in-k-group-approach2.ts?raw';
import approach2GoCode from './025-reverse-nodes-in-k-group-approach2.go?raw';
...
<Tabs>
  ...
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="reverse-nodes-in-k-group-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="025-reverse-nodes-in-k-group-swift" />
  </TabItem>
</Tabs>
...
func reverseKGroup(head *ListNode, k int) *ListNode {
    var values []int
    cur := head
    for cur != nil { values = append(values, cur.Val); cur = cur.Next } // L1: O(1) per node
    n := len(values)
    for i := 0; i+k <= n; i += k {                                      // L2: O(k) per group
        lo, hi := i, i+k-1
        for lo < hi { values[lo], values[hi] = values[hi], values[lo]; lo++; hi-- }
    }
    dummy := &ListNode{}
    tail := dummy
    for _, v := range values { tail.Next = &ListNode{Val: v}; tail = tail.Next } // L3
    return dummy.Next
}
```
</TabItem>
<TabItem label="Swift">
  <SwiftRepl code={approach1SwiftCode} id="025-reverse-nodes-in-k-group-approach1-swift" />
</TabItem>
</Tabs>
...
func reverseKGroup(head *ListNode, k int) *ListNode {
    ...
    for {
        // 1. Find the k-th node from groupPrev
        kth := groupPrev
        for i := 0; i < k; i++ {            // L1: advance k steps to find group end
            kth = kth.Next
        ...
        for curr != groupNext {              // L2: reverse k links
            nxt := curr.Next
            curr.Next = prev                 // L3: O(1) pointer reversal
            prev = curr
            curr = nxt
...
<TabItem label="Swift">
  <SwiftRepl code={approach2SwiftCode} id="025-reverse-nodes-in-k-group-approach2-swift" />
</TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach2PyCode} id="reverse-nodes-in-k-group-approach2-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach2TsCode} id="reverse-nodes-in-k-group-approach2-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="reverse-nodes-in-k-group-approach2-go" />
  </TabItem>
</Tabs>
...
<TabItem label="Swift">
  <SwiftRepl code={approach3SwiftCode} id="025-reverse-nodes-in-k-group-approach3-swift" />
</TabItem>
</Tabs>
