...
func copyRandomList(_ head: Node?) -> Node? {
        let originals = nodes(head)
        let copies = originals.map { Node($0.val) }
        for index in copies.indices {
            if index + 1 < copies.count {
                copies[index].next = copies[index + 1]
            }
            if let random = originals[index].random,
               let randomIndex = originals.firstIndex(where: { $0 === random }) {
                copies[index].random = copies[randomIndex]
            }
        }
        return copies.first
    }

    private func nodes(_ head: Node?) -> [Node] {
        var result: [Node] = []
        var current = head
        while let node = current {
            result.append(node)
