...
import practiceCodeTs from './146-lru-cache.ts?raw';
import practiceCodeGo from './146-lru-cache.go?raw';
import practiceCodeSwift from './146-lru-cache.swift?raw';
import approach1SwiftCode from './146-lru-cache-approach1.swift?raw';
import approach2SwiftCode from './146-lru-cache-approach2.swift?raw';
import approach3SwiftCode from './146-lru-cache-approach3.swift?raw';
import approach3PyCode from './146-lru-cache-approach3.py?raw';
import approach3TsCode from './146-lru-cache-approach3.ts?raw';
import approach3GoCode from './146-lru-cache-approach3.go?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="lru-cache-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="lru-cache-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="lru-cache-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="146-lru-cache-swift" />
  </TabItem>
</Tabs>
...
class LRUCache {
    ...
    private order: number[];  // least recent first

    constructor(capacity: number) {
        this.cap = capacity; this.data = new Map(); this.order = [];
    }

    get(key: number): number {
        if (!this.data.has(key)) return -1;
        this.order.splice(this.order.indexOf(key), 1);  // L1: O(n) linear scan
        this.order.push(key);                            // L2: O(1) append
        return this.data.get(key)!;
    }

    put(key: number, value: number): void {
        if (this.data.has(key)) {
            this.order.splice(this.order.indexOf(key), 1);  // L3: O(n) remove existing
        } else if (this.data.size >= this.cap) {
            const evict = this.order.shift()!;               // L4: O(n) shift all elements
...
<TabItem label="Swift">
  <SwiftRepl code={approach1SwiftCode} id="146-lru-cache-approach1-swift" />
</TabItem>
</Tabs>
...
<TabItem label="TypeScript">
```typescript
// JavaScript Map preserves insertion order; simulate move_to_end by
// deleting and re-inserting, both O(1) amortized.
class LRUCache {
    private cap: number;
    ...
    get(key: number): number {
        ...
        this.cache.delete(key); this.cache.set(key, val);  // L1: O(1) move to end
        return val;
    }

    put(key: number, value: number): void {
        if (this.cache.has(key)) this.cache.delete(key);  // L2: O(1) remove existing
        this.cache.set(key, value);
        if (this.cache.size > this.cap) {
            this.cache.delete(this.cache.keys().next().value!);  // L3: O(1) pop LRU
        }
    }
}
```
...
<TabItem label="Swift">
  <SwiftRepl code={approach2SwiftCode} id="146-lru-cache-approach2-swift" />
</TabItem>
</Tabs>
...
class LRUCache {
    ...
    private addToFront(node: LRUNode): void {
        ...
        this.head.next!.prev = node;         // L3: O(1) insert at head
        this.head.next = node;               // L4: O(1) insert at head
    }

    get(key: number): number {
        const node = this.cache.get(key);
        if (!node) return -1;
        this.remove(node); this.addToFront(node);  // L5-L7: O(1)
        return node.val;
    }
    ...
    put(key: number, value: number): void {
        ...
        if (existing) {
            existing.val = value;
            this.remove(existing); this.addToFront(existing); return;  // L8-L9
        }
        if (this.cache.size >= this.cap) {
            const lru = this.tail.prev!;
            this.remove(lru); this.cache.delete(lru.key);              // L10
        }
        const node = new LRUNode(key, value);
        this.cache.set(key, node); this.addToFront(node);             // L11
    }
}
```
</TabItem>
<TabItem label="Swift">
  <SwiftRepl code={approach3SwiftCode} id="146-lru-cache-approach3-swift" />
</TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach3PyCode} id="lru-cache-approach3-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach3TsCode} id="lru-cache-approach3-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="lru-cache-approach3-go" />
  </TabItem>
...
class LRUNode {
    key: number; val: number;
    prev: LRUNode | null = null; next: LRUNode | null = null;
    constructor(key = 0, val = 0) { this.key = key; this.val = val; }
}

class LRUCache {
    private cap: number; private cache: Map<number, LRUNode>;
    private head: LRUNode; private tail: LRUNode;
    constructor(capacity: number) {
        this.cap = capacity; this.cache = new Map();
        this.head = new LRUNode(); this.tail = new LRUNode();
        this.head.next = this.tail; this.tail.prev = this.head;
    }
    private remove(node: LRUNode): void { node.prev!.next = node.next; node.next!.prev = node.prev; }
    private addToFront(node: LRUNode): void {
        node.prev = this.head; node.next = this.head.next;
        this.head.next!.prev = node; this.head.next = node;
    }
    get(key: number): number {
        const node = this.cache.get(key); if (!node) return -1;
        this.remove(node); this.addToFront(node); return node.val;
    }
    put(key: number, value: number): void {
        const existing = this.cache.get(key);
        if (existing) { existing.val = value; this.remove(existing); this.addToFront(existing); return; }
        if (this.cache.size >= this.cap) { const lru = this.tail.prev!; this.remove(lru); this.cache.delete(lru.key); }
        const node = new LRUNode(key, value); this.cache.set(key, node); this.addToFront(node);
    }
}
...
cache.put(3, 3); console.assert(cache.get(2) === -1);
cache.put(4, 4); console.assert(cache.get(1) === -1);
console.assert(cache.get(3) === 3); console.assert(cache.get(4) === 4);
const c1 = new LRUCache(1);
c1.put(1, 10); console.assert(c1.get(1) === 10);
c1.put(2, 20); console.assert(c1.get(1) === -1); console.assert(c1.get(2) === 20);
const c2 = new LRUCache(2);
c2.put(1, 1); c2.put(2, 2); c2.put(1, 100); c2.put(3, 3);
console.assert(c2.get(1) === 100); console.assert(c2.get(2) === -1); console.assert(c2.get(3) === 3);
