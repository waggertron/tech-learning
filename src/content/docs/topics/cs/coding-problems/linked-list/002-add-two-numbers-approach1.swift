...
func addTwoNumbers(_ l1: ListNode?, _ l2: ListNode?) -> ListNode? {
        let left = decimalString(l1)
        let right = decimalString(l2)
        return makeList(Array(addDecimalStrings(left, right).reversed()).compactMap {
            $0.wholeNumberValue
        })
    }

    private func decimalString(_ head: ListNode?) -> String {
        listValues(head).reversed().map(String.init).joined()
    }

    private func addDecimalStrings(_ left: String, _ right: String) -> String {
        let a = Array(left)
        let b = Array(right)
        var i = a.count - 1
        var j = b.count - 1
        ...
        while i >= 0 || j >= 0 || carry > 0 {
            let leftDigit = i >= 0 ? a[i].wholeNumberValue ?? 0 : 0
            let rightDigit = j >= 0 ? b[j].wholeNumberValue ?? 0 : 0
            let sum = leftDigit + rightDigit + carry
            reversedDigits.append(String(sum % 10))
            carry = sum / 10
            i -= 1
            j -= 1
        }
        return reversedDigits.reversed().joined()
    }
}
