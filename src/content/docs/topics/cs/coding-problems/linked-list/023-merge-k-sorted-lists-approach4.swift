...
final class Solution {
    func mergeKLists(_ lists: [ListNode?]) -> ListNode? {
        var heap = BinaryHeap<ListHeapEntry> { left, right in
            left.value == right.value ? left.serial < right.serial : left.value < right.value
        }
        var serial = 0
        for case let node? in lists {
            heap.insert(ListHeapEntry(value: node.val, serial: serial, node: node))
            serial += 1
        }
        let dummy = ListNode()
        var tail = dummy
        while let entry = heap.removeRoot() {
            tail.next = entry.node
            tail = entry.node
            if let next = entry.node.next {
                heap.insert(ListHeapEntry(value: next.val, serial: serial, node: next))
                serial += 1
            }
        }
        tail.next = nil
