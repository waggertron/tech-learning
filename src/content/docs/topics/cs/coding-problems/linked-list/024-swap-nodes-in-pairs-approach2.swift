...
func runTests() {
    // TEST_VECTORS_BEGIN sha256:d441aadf5983030d141da1d3ae1fcdead6695e9acc3614dbf44f9e832eea8a07
    let argument1Case1 = makeList([1, 2, 3, 4])
    expectEqual(listValues(Solution().swapPairs(argument1Case1)), [2, 1, 4, 3], "even-length")
    let argument1Case2 = makeList([1, 2, 3, 4, 5])
    expectEqual(listValues(Solution().swapPairs(argument1Case2)), [2, 1, 4, 3, 5], "odd-length")
    let argument1Case3 = makeList([7, 8])
    expectEqual(listValues(Solution().swapPairs(argument1Case3)), [8, 7], "two-nodes")
    let argument1Case4 = makeList([])
    expectEqual(listValues(Solution().swapPairs(argument1Case4)), [], "empty-list")
    // EXCLUDED_VECTOR value-out-of-range: [[101]] | Node values stay within the published range.
    // TEST_VECTORS_END
    reportSuccess()
}
