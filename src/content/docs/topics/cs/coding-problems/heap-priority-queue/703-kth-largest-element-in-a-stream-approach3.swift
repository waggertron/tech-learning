...
final class KthLargest {
    ...
    init(_ k: Int, _ nums: [Int]) { self.k = k; for value in nums { heap.insert(value); if heap.count > k { _ = heap.removeRoot() } } }
    func add(_ val: Int) -> Int { heap.insert(val); if heap.count > k { _ = heap.removeRoot() }; return heap.peek! }
}
