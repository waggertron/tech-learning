...
final class Solution {
    func kClosest(_ points: [[Int]], _ k: Int) -> [[Int]] {
        var values = points; var low = 0; var high = values.count - 1
        while low <= high { let pivot = partition(&values, low, high); if pivot == k - 1 { break }; if pivot < k - 1 { low = pivot + 1 } else { high = pivot - 1 } }
        return Array(values.prefix(k)).sorted(by: isCloser)
    }
    private func partition(_ values: inout [[Int]], _ low: Int, _ high: Int) -> Int { let pivot = values[high]; var store = low; for index in low..<high where isCloser(values[index], pivot) { values.swapAt(store, index); store += 1 }; values.swapAt(store, high); return store }
    private func isCloser(_ a: [Int], _ b: [Int]) -> Bool { let da = a[0] * a[0] + a[1] * a[1]; let db = b[0] * b[0] + b[1] * b[1]; return da == db ? (a[0] == b[0] ? a[1] < b[1] : a[0] < b[0]) : da < db }
}
