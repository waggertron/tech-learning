...
final class MedianFinder {
    ...
    func addNum(_ num: Int) {
        ...
        while low < high { let middle = (low + high) / 2; if values[middle] < num { low = middle + 1 } else { high = middle } }
        values.insert(num, at: low)
    }
    func findMedian() -> Double {
        let middle = values.count / 2
        if values.count % 2 == 1 { return Double(values[middle]) }
