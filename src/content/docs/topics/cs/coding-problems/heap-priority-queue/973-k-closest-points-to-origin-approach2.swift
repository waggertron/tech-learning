...
private struct PointEntry { let point: [Int]; let distance: Int }
final class Solution {
    func kClosest(_ points: [[Int]], _ k: Int) -> [[Int]] {
        var heap = BinaryHeap<PointEntry> { $0.distance != $1.distance ? $0.distance > $1.distance : ($0.point[0] != $1.point[0] ? $0.point[0] > $1.point[0] : $0.point[1] > $1.point[1]) }
        for point in points { heap.insert(PointEntry(point: point, distance: point[0] * point[0] + point[1] * point[1])); if heap.count > k { _ = heap.removeRoot() } }
        var result: [[Int]] = []; while let entry = heap.removeRoot() { result.append(entry.point) }; return result.sorted(by: isCloser)
    }
