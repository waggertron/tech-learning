...
import practiceCode from './295-find-median-from-data-stream.py?raw';
import practiceCodeTs from './295-find-median-from-data-stream.ts?raw';
import practiceCodeGo from './295-find-median-from-data-stream.go?raw';
import practiceCodeSwift from './295-find-median-from-data-stream.swift?raw';
import approach3TsCode from './295-find-median-from-data-stream-approach3.ts?raw';
import approach3GoCode from './295-find-median-from-data-stream-approach3.go?raw';
import approach1SwiftCode from './295-find-median-from-data-stream-approach1.swift?raw';
import approach2SwiftCode from './295-find-median-from-data-stream-approach2.swift?raw';
import approach3SwiftCode from './295-find-median-from-data-stream-approach3.swift?raw';
...
<Tabs>
  ...
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="find-median-from-data-stream-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="295-find-median-from-data-stream-swift" />
  </TabItem>
</Tabs>
...
func (mf *MedianFinder) AddNum(num int) {
    mf.nums = append(mf.nums, num) // L1: O(1) append
}

func (mf *MedianFinder) FindMedian() float64 {
    s := make([]int, len(mf.nums))
    copy(s, mf.nums)
    sort.Ints(s)           // L2: O(n log n) sort on every query
    n := len(s)
    if n%2 == 1 { return float64(s[n/2]) }
    return float64(s[n/2-1]+s[n/2]) / 2
}
```
</TabItem>
<TabItem label="Swift">
  <SwiftRepl code={approach1SwiftCode} id="295-find-median-from-data-stream-approach1-swift" />
</TabItem>
</Tabs>
...
func (mf *MedianFinder) AddNum(num int) {
    // Binary search for insertion point, then insert
    i := sort.SearchInts(mf.nums, num)
    mf.nums = append(mf.nums, 0)
    copy(mf.nums[i+1:], mf.nums[i:]) // L1: O(log n) search + O(n) shift
    mf.nums[i] = num
}

func (mf *MedianFinder) FindMedian() float64 {
    n := len(mf.nums)
    if n%2 == 1 { return float64(mf.nums[n/2]) } // L2: O(1) index
    return float64(mf.nums[n/2-1]+mf.nums[n/2]) / 2
}
```
</TabItem>
<TabItem label="Swift">
  <SwiftRepl code={approach2SwiftCode} id="295-find-median-from-data-stream-approach2-swift" />
</TabItem>
</Tabs>
...
func Constructor() MedianFinder {
    lo := &MaxHeap{}; hi := &MinHeap{}; heap.Init(lo); heap.Init(hi)
...
func (mf *MedianFinder) FindMedian() float64 {
    if mf.lo.Len() > mf.hi.Len() { return float64((*mf.lo)[0]) } // L4: O(1)
    return float64((*mf.lo)[0]+(*mf.hi)[0]) / 2                   // L5: O(1)
}
...
func runTests() {
    mf := Constructor(); mf.AddNum(1); mf.AddNum(2)
    assert(mf.FindMedian() == 1.5); mf.AddNum(3); assert(mf.FindMedian() == 2.0)
    mf2 := Constructor(); mf2.AddNum(42); assert(mf2.FindMedian() == 42.0)
    mf3 := Constructor()
    for _, v := range []int{5, 3, 8, 1, 9} { mf3.AddNum(v) }
    assert(mf3.FindMedian() == 5.0)
    mf4 := Constructor()
    for _, v := range []int{2, 4, 6, 8} { mf4.AddNum(v) }
...
<TabItem label="Swift">
  <SwiftRepl code={approach3SwiftCode} id="295-find-median-from-data-stream-approach3-swift" />
</TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="Go">
    <GoRepl code={approach3GoCode} id="find-median-approach3-go" />
  </TabItem>
</Tabs>
