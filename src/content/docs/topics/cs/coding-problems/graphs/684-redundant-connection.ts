...
function _runTests(): void {
    ...
    // perf
    const bigEdges = Array.from({ length: 9_999 }, (_, i) => [i + 1, i + 2]);
    bigEdges.push([1, 10_000]);
    const t0 = performance.now();
    findRedundantConnection(bigEdges);
    console.log(`perf redundant-connection 10000 nodes cycle: ${(performance.now() - t0).toFixed(1)}ms`);
