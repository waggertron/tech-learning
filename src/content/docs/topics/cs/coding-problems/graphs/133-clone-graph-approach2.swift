...
final class Solution {
    func cloneGraph(_ node: Node?) -> Node? {
        ...
        var copies = [ObjectIdentifier(node): root]
        var queue = [node], head = 0
        while head < queue.count {
            let current = queue[head]; head += 1
            let copy = copies[ObjectIdentifier(current)]!
            for neighbor in current.neighbors {
                guard let neighbor else { copy.neighbors.append(nil); continue }
                let key = ObjectIdentifier(neighbor)
                if copies[key] == nil { copies[key] = Node(neighbor.val); queue.append(neighbor) }
