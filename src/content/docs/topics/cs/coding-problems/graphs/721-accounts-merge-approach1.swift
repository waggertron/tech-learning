...
final class Solution {
    func accountsMerge(_ accounts: [[String]]) -> [[String]] {
        var unionFind = UnionFind(accounts.count), owner: [String: Int] = [:]
        for index in accounts.indices {
            for email in accounts[index].dropFirst() {
                if let previous = owner[email] { _ = unionFind.union(index, previous) } else { owner[email] = index }
            }
        }
        var groups: [Int: [String]] = [:]
        for email in owner.keys.sorted() { groups[unionFind.find(owner[email]!), default: []].append(email) }
        return groups.map { root, emails in [accounts[root][0]] + emails }
