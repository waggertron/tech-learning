...
<PythonRepl code={practiceCode} id="possible-bipartition" />
...
Each person is dequeued at most once (L13 only fires when `neighbor not in color`). Each dislike edge is examined twice (once from each endpoint), so L12 totals O(e) across the full BFS.
