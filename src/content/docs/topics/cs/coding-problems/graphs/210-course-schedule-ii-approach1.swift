...
final class Solution {
    func findOrder(_ numCourses: Int, _ prerequisites: [[Int]]) -> [Int] {
        var requires = Array(repeating: [Int](), count: numCourses)
        for edge in prerequisites { requires[edge[0]].append(edge[1]) }
        var state = Array(repeating: 0, count: numCourses)
        func feasible(_ course: Int) -> Bool {
            if state[course] == 1 { return false }
            if state[course] == 2 { return true }
            state[course] = 1
            for prerequisite in requires[course] where !feasible(prerequisite) { return false }
            state[course] = 2
            return true
        }
        guard (0..<numCourses).allSatisfy(feasible) else { return [] }
