...
import practiceCode from './127-word-ladder.py?raw';
import practiceCodeTs from './127-word-ladder.ts?raw';
import practiceCodeGo from './127-word-ladder.go?raw';
import practiceCodeSwift from './127-word-ladder.swift?raw';
import approach1SwiftCode from './127-word-ladder-approach1.swift?raw';
import approach2SwiftCode from './127-word-ladder-approach2.swift?raw';
import approach3SwiftCode from './127-word-ladder-approach3.swift?raw';
import approach2PyCode from './127-word-ladder-approach2.py?raw';
import approach2TsCode from './127-word-ladder-approach2.ts?raw';
import approach2GoCode from './127-word-ladder-approach2.go?raw';
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={practiceCode} id="word-ladder-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={practiceCodeTs} id="word-ladder-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={practiceCodeGo} id="word-ladder-go" />
  </TabItem>
  <TabItem label="Swift">
    <SwiftRepl code={practiceCodeSwift} id="127-word-ladder-swift" />
  </TabItem>
</Tabs>
...
function ladderLength(beginWord: string, endWord: string, wordList: string[]): number {
    ...
    if (!words.has(endWord)) return 0;      // L1: O(N) set lookup

    const L = beginWord.length;
    const q: [string, number][] = [[beginWord, 1]];  // L3: init queue
    const visited = new Set([beginWord]);   // L4: init visited

    function oneAway(a: string, b: string): boolean {  // L5: O(L) per call
        let diff = 0;
    ...
    let head = 0;
    while (head < q.length) {              // L6: outer BFS loop
        const [word, dist] = q[head++];    // L7: O(1)
        if (word === endWord) return dist;
        for (const cand of words) {        // L8: O(N) scan per word
            if (!visited.has(cand) && oneAway(word, cand)) {  // L9: O(L)
                visited.add(cand);
                q.push([cand, dist + 1]);
            }
        }
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach1SwiftCode} id="127-word-ladder-approach1-swift" />
  </TabItem>
</Tabs>
...
```typescript
function ladderLength(beginWord: string, endWord: string, wordList: string[]): number {
    if (!wordList.includes(endWord)) return 0;       // L1: O(N)

    const L = beginWord.length;                      // L2: O(1)
    const patterns = new Map<string, string[]>();    // L3: O(1) init
    for (const w of wordList) {                      // L4: O(N) outer
        for (let i = 0; i < L; i++) {                // L5: O(L) inner
            const key = w.slice(0, i) + '*' + w.slice(i + 1);  // L6: O(L) slice + store
            if (!patterns.has(key)) patterns.set(key, []);
            patterns.get(key)!.push(w);
        }
    }

    const q: [string, number][] = [[beginWord, 1]]; // L7: O(1)
    const visited = new Set([beginWord]);            // L8: O(1)
    let head = 0;
    while (head < q.length) {                        // L9: BFS loop
        const [word, dist] = q[head++];              // L10: O(1)
        if (word === endWord) return dist;
        for (let i = 0; i < L; i++) {                // L11: O(L) per word
            const key = word.slice(0, i) + '*' + word.slice(i + 1);  // L12: O(L) slice
            for (const nb of patterns.get(key) ?? []) {  // L13: O(neighbors)
                if (!visited.has(nb)) {
                    visited.add(nb);
                    q.push([nb, dist + 1]);
                }
            }
            patterns.set(key, []);                   // L14: prune bucket
        }
    }
    return 0;
}
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach2SwiftCode} id="127-word-ladder-approach2-swift" />
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Python">
    <PythonRepl code={approach2PyCode} id="word-ladder-approach2-py" />
  </TabItem>
  <TabItem label="TypeScript">
    <TypeScriptRepl code={approach2TsCode} id="word-ladder-approach2-ts" />
  </TabItem>
  <TabItem label="Go">
    <GoRepl code={approach2GoCode} id="word-ladder-approach2-go" />
  </TabItem>
</Tabs>
...
```typescript
function ladderLength(beginWord: string, endWord: string, wordList: string[]): number {
    const words = new Set(wordList);            // L1: O(N) build set
    if (!words.has(endWord)) return 0;          // L2: O(1)

    const L = beginWord.length;                 // L3: O(1)
    let front = new Set([beginWord]);           // L4: O(1) init frontiers
    let back = new Set([endWord]);
    const visited = new Set([beginWord, endWord]);  // L5: O(1)
    let dist = 1;                               // L6: O(1)

    while (front.size > 0 && back.size > 0) {  // L7: outer loop, O(diameter) levels
        if (front.size > back.size) {           // L8: swap to keep front smaller
            [front, back] = [back, front];
        }

        const nextFront = new Set<string>();
        for (const w of front) {               // L9: iterate smaller frontier
            for (let i = 0; i < L; i++) {      // L10: O(L) positions
                for (let code = 97; code <= 122; code++) {  // L11: O(26) chars
                    const cand = w.slice(0, i) + String.fromCharCode(code) + w.slice(i + 1);  // L12
                    if (back.has(cand)) return dist + 1;     // L13: O(1) set lookup
                    if (words.has(cand) && !visited.has(cand)) {  // L14: O(1)
                        visited.add(cand);
                        nextFront.add(cand);
                    }
                }
...
```
  ...
  <TabItem label="Swift">
    <SwiftRepl code={approach3SwiftCode} id="127-word-ladder-approach3-swift" />
  </TabItem>
</Tabs>
...
console.assert(ladderLength("hit", "cog", ["hot","dot","dog","lot","log","cog"]) === 5);
console.assert(ladderLength("hit", "cot", ["hot","dot","dog","lot","log","cog"]) === 0);
console.assert(ladderLength("hot", "dot", ["dot","lot"]) === 2);
console.assert(ladderLength("hit", "cog", ["hot","dot","dog","lot","log"]) === 0);
console.assert(ladderLength("a", "c", ["a","b","c"]) === 2);
...
- [BFS](../../../coding-concepts/bfs/), breadth-first traversal tactics for level order, shortest unweighted paths, and expanding frontiers.
