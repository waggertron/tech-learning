...
<PythonRepl code={practiceCode} id="accounts-merge" />
...
The sort in L15 dominates the Union-Find operations because alpha is effectively constant while log k grows. Total email count is n * k, and sorting all emails across all groups costs O(n * k * log k).
