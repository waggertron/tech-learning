...
**Semicolons joining independent clauses.** Academic writing, legal documents, and technical documentation use semicolons to link two closely related independent clauses. These registers dominate formal pretraining corpora: Wikipedia, papers, textbooks, documentation. The model absorbed the pattern and applies it as a default way to connect two related thoughts without committing to a stronger logical connector ("because," "therefore," "but"). In informal and conversational writing — blogs, emails, spoken transcription — semicolons between independent clauses are vanishingly rare. Every instance can be replaced by a period, a comma with a conjunction, or an explicit logical connector, all of which read more naturally in non-formal prose.
