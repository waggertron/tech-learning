...
The model sees tools through their name, description, and argument schema. Everything else (how the tool is implemented, what it actually does) is invisible until a result comes back. That makes tool design a UX problem for the model, not just an [API design](../../system-design/api-design/) problem.
