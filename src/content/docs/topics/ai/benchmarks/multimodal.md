...
**Reasoning mode applied to vision (2025).** The same RL-trained reasoning that improved math benchmarks improved visual reasoning on MathVista and MMMU-Pro. The model can now reason: "the graph appears to cross zero at x~2.5; let me verify by extrapolating from the nearby labeled points." This added ~10-20 points on hard multimodal benchmarks. ChartQA and DocVQA (which are closer to retrieval than reasoning) benefited less.
