...
- **[Skill vs. tool vs. agent vs. prompt](skill-vs-tool-vs-agent/)**: Prompts (CLAUDE.md) = always-on behavior. Tools = atomic executable functions with side effects. Skills = reusable procedure packages, loaded on demand. Agents = autonomous multi-step executors orchestrating the others.
